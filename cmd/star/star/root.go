...
// Package star is star's command tree: [NewRootCmd] builds the root on the shared root of
// [cli.NewRootCmd], attaches the `key` and `docs` commands, and registers every command the bundled Starlark
// extensions contribute. It is a package rather than part of `main` so the reference generator can walk the
// tree in-process, as it walks writ's and lore's (#787).
package star
...
import (
	...
	// The inventories register every provider at init; the tree's commands, and the extensions' Starlark, run
	// against them, so the root package carries the registrations wherever it is imported.
...
// star copies `--dry-run` into [extension.DryRun], refreshes the application's flag map from parsed
...
Generate shell completions with:
  ...
  star completion fish > ~/.config/fish/completions/star.fish`,
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		extension.DryRun = assert.Must(cmd.Flags().GetBool("dry-run"))
		runtime.Refresh(cmd)
...
func loadStarlarkCommands(rootCmd *cobra.Command, runtime *extension.Application) error {
	// The bundled extensions: one directory per extension at the root of the embedded filesystem.
	loader := extension.NewLoader(bundled.FS)
...
func registerStarlarkCommand(rootCmd *cobra.Command, cmd *extension.Command) error {
...
func useLineFor(leafName string, args []extension.Arg) string {
...
func argsValidatorFor(args []extension.Arg) cobra.PositionalArgs {
...
func collectFlagValues(c *cobra.Command, flags []extension.Flag) map[string]string {
...
func defineFlags(rootCmd, cobraCmd *cobra.Command, flags []extension.Flag) error {
