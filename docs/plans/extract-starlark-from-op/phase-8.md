...
| # | Step | Status | Notes |
|---|---|---|---|
| 1 | Invocation registry + options types + plan.options builder | incomplete — pending tests | Code present: `op.Invocation` (`pkg/op/invocation.go:18`) + `op.InvocationRegistry` (`pkg/op/invocation_registry.go:18` — `All` / `ByLabel` / `AutoLabel` / `Register` / `Reset`), in `pkg/op` (not `starlarkbridge` as drafted); the `Options` / `plan.options` builder was removed (no `Options` type; not announced). **No behavioral test exists** — `go test ./pkg/op/ -run 'Invocation'` → "no tests to run"; the only `_test.go` mention is a codegen name-assertion (`plan/gen/receiver_type.gen_test.go:120`). **Reverted complete → incomplete (2026-06-15)** pending test development: registry behavior (ordering, labeling, dedup, reset) is unverified. **Tests to write** — `pkg/op/invocation_registry_test.go`: `TestInvocationRegistry_New_IsEmpty`, `TestInvocationRegistry_Register_AppendsInCreationOrder`, `TestInvocationRegistry_Register_IndexesByLabel`, `TestInvocationRegistry_Register_RejectsDuplicateLabel`, `TestInvocationRegistry_ByLabel_ReturnsNilForUnknown`, `TestInvocationRegistry_AutoLabel_IncrementsPerProviderMethod`, `TestInvocationRegistry_All_ReturnsIndependentCopy`, `TestInvocationRegistry_Reset_ClearsEntriesAndCounters`, `TestInvocationRegistry_Concurrent_IsRaceFree` (`-race`); `pkg/op/invocation_test.go`: `TestInvocation_SlotValue_DelegatesToResultPromise`. |
...
| 10.0(h) | Post-13.0(b) cleanup | complete | `wrapper`→`goReceiver` rename; `file.Receipt.recoveryID` field; stack-comparison docs refreshed; bare-UUID test sweep (9 sites); `ui.Provider` test pull. |
...
| 11 | plan.gather concurrent-iteration combinator | complete (core); compensation-unwind untested | `GatherPlanner` (`flow/planners.go:115`) materializes a `*op.Subgraph` bound to `flow.Gather`, adopting `body=` invocations as iteration-template children via `addBodyChildren` — the subgraph-child pattern `plan.choose` (step 10) still lacks. `flow.Provider.Gather` (`provider.go:181`) dispatches that one materialized body subgraph **once per item, concurrently**: per-dispatch frame minting (`buildIterationFrame(activation.Variables, item)` — fresh `item` binding, body not duplicated), bounded concurrency (`limit`-semaphore, `limit<=0`→`Platform.DefaultConcurrency()`), result collection by index, first-error `gatherCtx` cancel + LIFO unwind of completed iterations, per-iteration recovery nested into one returned stack (`CompensateGather`, `:299`). **The "redesign" landed** (Phase 5 of 13.0(n), ForEach-Object shape) and does not wait on choose. **Proven:** 7 tests — `test_gather_{basic,concurrency,advanced}.star` (~16 sub-scenarios: per-item dispatch, item+outer-flag frame visibility, frame hygiene, all limit modes, empty-items/empty-body, multi-node body, sequencing) + 2 Go compensate no-op guards + 2 gen (DryRun, CompensableInterface). **Gap:** the failure/compensation path (mid-flight error → cancel rest → LIFO-unwind completed side effects) has only nil/empty no-op-guard tests; `TestGather_FailureUnwindsCompletedIterations` is unwritten. Step doc: [phase-8/steps/11-plan-gather.md](phase-8/steps/11-plan-gather.md). |
...
**Status:** in-progress (Phase 6 of 10.0(n) closed 2026-05-24; phase-8 exit pending steps 11, 12, 18–26 + O1/O2/O3).
...
- **Steps 16, 17, 18, 19, 20** complete; closed inline as part of the 13.0(n) and follow-on commits.
  **Steps 21, 22** + new step 23 + open design items O1–O3 remain. Step 23 is the explicit phase-8 PR
  gate — phase-8 is not PR-eligible to develop until step 23 closes AND the full `make test` suite is
  green.
- **Step 23 — Framework helper direct-test backfill + PR gate.** Phase 6.0's convertibility-layer
  helpers (`typesAreInterconvertible` / `sourceSideAdvertises` / `targetSideAdvertises` / `mergeBubbled`
  / `preferSourceSide`) and step 19's `checkPromiseTypes` plus the pre-existing `Method.ResultType` all
  landed with zero direct unit tests, relying on indirect integration coverage. Step 23 closes that gap
  ...
  plumbing.
- **Step 21 — graph immutability + test triage.** Framework half **landed** (2026-05-27): `Graph`
...
- **Step 22 — `file.Resource` taxonomic split** (`file.Regular` / `file.Directory` / `file.Link`).
- **Step 24 (table row) — PowerShell naming standardization.** Added 2026-06-17.
  ...
  [phase-8/steps/24-powershell-naming-standardization.md](phase-8/steps/24-powershell-naming-standardization.md).
- **Step 25 (table row) — Relocate RuntimeEnvironment from providers to resources.** Added 2026-06-18.
  ...
  marshalers. Gated on step 23. Step doc:
  [phase-8/steps/25-relocate-env-provider-to-resource.md](phase-8/steps/25-relocate-env-provider-to-resource.md).
- **Step 26 (table row) — Caller id on the activation.** Added 2026-06-18. The activation carries a
  ...
  [phase-8/steps/26-starlark-callsite-unit-id.md](phase-8/steps/26-starlark-callsite-unit-id.md).
- **Numbering note (2026-06-17):** the step numbers in this bulleted section predate the audit's table
  renumbering and no longer align with the table above (e.g. this section's "Step 23 helper-backfill" is
  table row 20). The table is authoritative; reconciling this prose is a pending tidy-up.
...
in-flight scaffolding from step 21's graph-immutability work added a `Snapshot` type in
`pkg/op/snapshot.go` with `GraphExecutor.Snapshot()` and a `ResumeExecutor(graph, spec, snapshot)`
constructor; the rename lands as part of step 21:
