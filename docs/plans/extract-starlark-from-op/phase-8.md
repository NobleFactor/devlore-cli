...
| 13.0(n) | Parameter binding infrastructure | in-progress (plan-doc drafted) | Reframed from "create writ graph executor" to the broader binding-model overhaul that the original scope implied. **Scope:** new `ParameterValue{Name string}` slot variant on the sealed `op.SlotValue` interface (third deferred-resolution variant alongside `ImmediateValue` and `PromiseValue`); `op.Parameter.Default *any` for required-vs-optional discrimination; `ExecutableUnit.Parameters()` bubble-up surface (node = method params with `ParameterValue` slots; subgraph = deduplicated union of topological-root parameters); new `pkg/binding.PropertyResolver` with layered source precedence (caller > program env `DEVLORE_<PROG>_<KEY>` > global env `DEVLORE_<KEY>` > config > default), origin tracking via unexported `property` struct in a single internal map, no exported fields; preflight pass `bindParameters` aggregating missing-required/type-mismatch/default-type-mismatch errors into the D5 envelope alongside `ResolvePending`; retirement of `op.EnvironmentValue`, `op.Properties`, `RuntimeEnvironment.Data`, `RuntimeEnvironment.Property`. **Integration target: `writ adopt`** — the three nil-activation call sites in `cmd/writ/writ/commands.go:1400/1410/1422` rewire to graph construction + `PropertyResolver` + `GraphExecutor.Run`. **Test-first:** starlark integration test under `cmd/devlore-test/testdata/binding/` modeling adopt's mkdir → move → link sequence with `plan.param()` declarations is written and intentionally fails before any infrastructure phase begins. **Exit criterion:** full `make test` green (including the new integration test) and `writ adopt` runs through the new infrastructure with identical observable behavior. **Out of scope for 13.0(n):** the 5 nil-activation sites in `cmd/writ/writ/migrate_cmd.go:292/302/310/320` and `migrate/execute.go:81`, plus the file provider's defensive nil-activation paths — deferred to a follow-on PR with the normal small-PR cadence. **One monster PR to develop at 13.0(n) close** (one-time exception driven by structural coupling of the slot-model change). Sub-plan: [phase-8/13.0-n.md](phase-8/13.0-n.md). |
...
binding infrastructure — slot-model `ParameterValue`, `pkg/binding.PropertyResolver`, bubble-up
