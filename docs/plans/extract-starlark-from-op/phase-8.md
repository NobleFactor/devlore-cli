...
| 39 | Complete per-provider design docs | in-progress — **7 of 14 landed**: 3.5.3 plan (2026-07-21), 3.5.4 file, batch 1 (3.5.5 json / 3.5.6 yaml / 3.5.7 template), batch 2 (3.5.8 shell — the deliberately non-compensable escape hatch; 3.5.9 powershell — `pwsh` hard-required, and the catalog's ONLY zero-coverage provider → step-52 intake rows 11–12) — each with greped test matrices, catalog rows, index entries; next: git/service, appnet/encryption/function, regexp/ui | 14 of 18 providers have no design doc; one 3.5.x doc + .status.md companion each, on the 3.5.2 pattern (every method covered, API surface, dual-API usage, verified per-method test matrix). Priority: plan (3.5.3), file (3.5.4), then the remaining planned/both providers. Charter: [phase-8/steps/39-per-provider-design-docs.md](phase-8/steps/39-per-provider-design-docs.md). |
