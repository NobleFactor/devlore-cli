...
| 13.0 | Resource foundation cleanup | in-progress | Prerequisite for step 13 and everything downstream that touches Resources. (a) Delete `<M>Planned` companions — code complete, doc closure folded into k.15. (b) Roll out 12 required Resource interfaces across all nine Resource-bearing providers — `op.ResourceBase` shared impls complete; per-type Digest/Etag/Addressing overrides complete for file/git/appnet/pkg; service + mem/json/yaml/function still inherit `ResourceBase` defaults (`AddressingUnknown` / `ErrUnimplemented`) and are pending k.6–k.11. |
...
| 13.0(e) | Saga shape and stack-based recovery | in-progress | Steps 1–5b complete (Action.FullName, RecoveryStack API, Method classifier + Invoke + pushComplement dispatcher, Tombstone→Receipt rename). Complement shape settled to `*op.RecoveryStack` as the singular legal form by 13.0(k) k.12. Closure-only `Push`/`Do` APIs already deleted from `RecoveryStack` (ahead of plan); remaining work is the orphaned file-provider bug list, which folds into k.14 caller migration sweep. |
...
`cmd/writ` block strict activation enforcement. Remaining within 13.0(k): k.12 boot-discipline test (now
viable; walks every announced Resource and asserts `Addressing() != AddressingUnknown`), catalog `Resolve`
rewrite, lifecycle integration, reconciliation pass. **(b)** Successor
