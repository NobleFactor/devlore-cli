...
| 34 | Architecture docs — rewrite `2.3`, `2`, `2.2` onto the `pkg/op` model in full | in-progress — slices A (`2-execution-graph.md`) + B (`2.2-phase-execution.md`: saga-over-units, receipt contract, recovery-stack tree; preserved step-41/42 sections; killed two banned `savedComplement` residuals) landed 2026-07-21, every claim tree-verified, status companions rewritten; slices C (`2.3`), D (reference sweep) pending | **Added 2026-06-20.** Rewrite 2 / 2.2 / 2.3 onto the sealed `op.Graph` model (no `Phase`/`RunPhased`/`SlotValue`); 2.3's dated section is the seed. Charter + scope note: [phase-8/steps/34-architecture-docs-rewrite.md](phase-8/steps/34-architecture-docs-rewrite.md). |
