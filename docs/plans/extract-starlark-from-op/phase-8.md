...
| 13.0 | Resource foundation cleanup | in-progress | Prerequisite for step 13 and everything downstream that touches Resources. (a) Delete `<M>Planned` companions — code complete, doc closure folded into k.15. (b) Roll out 12 required Resource interfaces across all eight types — `op.ResourceBase` shared impls complete; per-type overrides complete for file/git/appnet/pkg/service; CAS types (mem/json/yaml/function/stream) pending k.8–k.11. |
...
| 13.0(e) | Saga shape and stack-based recovery | in-progress | Steps 1–5b complete (Action.FullName, RecoveryStack API, Method classifier + Invoke + pushComplement, Tombstone→Receipt rename). Complement shape settled to `*op.RecoveryStack` as the singular legal form by 13.0(k) k.12. Deprecated closure-only `Push`/`Do` APIs and orphaned file-provider bug list fold into k.14 caller migration sweep. |
...
| 13.0(n) | Create writ graph executor | not-started | 7 callsites in `cmd/writ` invoke providers directly with `nil` activation. Blocks strict activation enforcement. When complete, writ commands run through a graph executor; `file.catalogProduct` drops its nil-check. |
