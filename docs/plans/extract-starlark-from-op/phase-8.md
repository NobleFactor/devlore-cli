...
| 34 | Architecture docs — rewrite `2.3`, `2`, `2.2` onto the `pkg/op` model in full | in-progress — all four slices landed 2026-07-21: A (`2-execution-graph.md`), B (`2.2-phase-execution.md`: saga-over-units, receipt contract, recovery-stack tree; killed two banned `savedComplement` residuals), C (`2.3-orchestration-primitives.md`: dated section promoted to the spine with its since-landed deviations updated — steps 31/35/42-3a; `Fatal`→`flow.failed`; `SlotProxy`→field projection), D (reference sweep: 7 docs spot-fixed incl. 3 more banned-`Complement` residuals; 13 structurally-stale docs inventoried in the step doc, not half-patched). Every claim tree-verified; status companions rewritten. **Closure pending one decision:** disposition of the 13-doc structural-debt inventory (charter a documentation-debt step vs. fold provider-adjacent ones into 39 vs. accept as recorded debt) | **Added 2026-06-20.** Rewrite 2 / 2.2 / 2.3 onto the sealed `op.Graph` model (no `Phase`/`RunPhased`/`SlotValue`); 2.3's dated section is the seed. Charter + scope note: [phase-8/steps/34-architecture-docs-rewrite.md](phase-8/steps/34-architecture-docs-rewrite.md). |
