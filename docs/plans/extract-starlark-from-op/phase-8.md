...
| 47 | writ deploy family — rewrite onto the sealed graph + the trace store (the StateView crater) | in-progress — slice 1 LANDED 2026-07-15 (internal/cli run index; the deploy package: per-scope plan.Provider graphs, four pipeline chains, pinning, WriteGraph+WriteTrace, integration tests; the readback package: time-ordered best-effort fold + tests; runDeployV2 rewired thin; framework fix rode along: linkChildren now preserves the document's child order for topological ties — every multi-branch document previously failed LoadGraph's integrity check); next: slice 2 (decommission + upgrade) | **Added 2026-07-15; formerly "step 33 slice C".** Four slices fed one at a time: (1) internal/cli NDJSON run index + the writ-owned readback (time-ordered best-effort fold) + deploy on the lore.Build pattern — Scenario 2's critical path; (2) decommission + upgrade (no signature gate — step 46 wires it; conservative drift interim); (3) `writ status` replaces reconcile (four sections; missing index = hard error; no --fix); (4) close — internal/execution + the inspect/list/receipt stubs deleted, docgen greens, **steps 18 + 22 close**. Step doc: [phase-8/steps/47-writ-deploy-family-rewrite.md](phase-8/steps/47-writ-deploy-family-rewrite.md) |
