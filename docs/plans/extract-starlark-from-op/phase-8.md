...
> that enforces it — is described in
> [PR #511](https://github.com/NobleFactor/devlore-cli/pull/511). Render the tree with
> `scripts/Get-EpicReport`; `--debt` ranks open bugs by severity.
>
> Every step below that is not complete carries its tracking issue in the Status column.
...
| 53 | Network-dependent tests make CI's red signal ambiguous | **[#439](https://github.com/NobleFactor/devlore-cli/issues/439)** · **charter 2026-08-14** — `TestRegistry_SyncIntegration` (`internal/lorepackage`) clones `https://github.com/NobleFactor/devlore-registry.git` from CI runners; on PR #414 the macOS runner failed DNS (`Could not resolve host: github.com`) and produced a red check unrelated to the change under review, after a 30s timeout. **Solution deliberately not chosen** — the charter requires enumerating every network-reaching test (by behavior, not by name) and measuring how often this actually fires before any fix is designed. Matters because the #405 campaign's discipline is a known-failure baseline verified name by name; a red check that sometimes means "the network was unwell" trains everyone to look past red. | Step doc: [phase-8/steps/53-network-dependent-tests.md](phase-8/steps/53-network-dependent-tests.md) |
| 54 | XDG anchors resolve to relative paths on Windows, and two packages disagree | **complete 2026-08-15** (this row read `charter` until 2026-08-18; the step doc has said complete since delivery) — chartered 2026-08-14 — `internal/cli/xdg.go` resolves every anchor from `os.Getenv("HOME")`, which Windows does not define, so with `XDG_*` unset `StateHome()` yields the RELATIVE `.local\state` and state lands under the process's working directory. `pkg/signing/configHome()` already does it correctly via `os.UserHomeDir()` — and its doc claims a parity with `internal/cli` that does not hold, so the same logical location resolves to two different places on Windows. CI cannot see it: scenario tests set `HOME` and `USERPROFILE`, and 65 `t.Setenv("XDG_*")` sites skip the fallback. **Blocks windows-native-permissions phase 3.1**, which would otherwise anchor writable roots at a relative path — worse than the `os.WriteFile` it replaces, because it looks confined. **Layout RULED 2026-08-14** after researching current practice: keep the four-way split rooted at the home directory on every platform, home resolved via `os.UserHomeDir()`. Nothing in the Known-Folder cohort (Neovim, gh, `adrg/xdg`, `platformdirs`, Rust `dirs`) treats `%APPDATA%` as a substitute for `$HOME` — they map XDG *roles* onto native folders — while Microsoft's own OpenSSH keeps private keys in `%USERPROFILE%\.ssh`, and both migration debates (starship#896, neovim#24009) sit open and unimplemented. Still open: the failure mode when home cannot resolve, and whether any cwd-relative data needs migrating. | Step doc: [phase-8/steps/54-xdg-anchors-on-windows.md](phase-8/steps/54-xdg-anchors-on-windows.md) |
| 55 | Curate `internal/` before it moves to `cmd/internal/` | **[#518](https://github.com/NobleFactor/devlore-cli/issues/518)** · **charter 2026-08-14** — a move is the moment to stop carrying things: rewriting 97 import lines is the cheapest chance to ask, per package, whether it should exist. Evidence already gathered: `internal/pwsh` has **zero importers, not even tests**, yet both package docs describe it as in use; `internal/console` is a second presentation layer with one consumer while everything else narrates through `pkg/status`; `internal/config` overlaps `pkg/devconfig`. `internal/document`'s disposition is settled (moves to `cmd/internal/document`) because its only `pkg/` dependency dissolved when `op` took trace persistence. Curate BEFORE the sweep — a package deleted after it pays the import rewrite twice. | Step doc: [phase-8/steps/55-curate-internal.md](phase-8/steps/55-curate-internal.md) |
| 56 | Rename `op` → `workflow`, then partition into `workflow` + `workflow/internal` | **[#451](https://github.com/NobleFactor/devlore-cli/issues/451)** · **queued 2026-08-14** — the rename itself is already planned in detail: [docs/plans/workflow-rename.md](../workflow-rename.md) (5 phases, ~200 importing files, ~2,499 qualified references, 29 generated files; `Node`→`Step`, `ExecutableUnit`→`Node`, `RecoveryStack`→`Ledger`, `Graph`→`Definition`, `GraphExecutor`→`Executor`, `Subgraph`→`Block`). **Three gaps found 2026-08-14:** (a) the plan is explicitly "a pure rename… no scope creep", so the `workflow/internal` PARTITION is not in it — that is a sixth phase with real design content, one judgment per exported symbol; (b) its stated prerequisite names an in-flight branch that no longer exists, so the gate is stale text rather than an unmet condition; (c) its documentation audit says "every file mentioning the old names" without enumerating them, while **12+ architecture docs** define the renamed concepts — an unenumerated audit has no completeness check. **Sequencing:** the rename is a serial block of five PRs during which any in-flight branch in `pkg/op/` conflicts irreparably, so it owns a window — after the windows campaign and after the `internal/` sweep. | Plan: [workflow-rename.md](../workflow-rename.md) |
| 57 | Mine the `workflow-rename-plan` worktree before retiring it | **[#517](https://github.com/NobleFactor/devlore-cli/issues/517)** · **charter 2026-08-14** — the worktree at `devlore-cli.workflow-rename-plan` is **local-only** (no remote branch) and three months stale (2026-05-26), yet holds content that exists nowhere else: a 257-line cross-repo consolidation analysis, a 270-line long-context-prompting draft, a 60-line **cache-augmented-generation** draft arguing a static domain should use context caching rather than a vector database (directly relevant to the registry's CAG assets), a 12-file `prototype/bindgen` CLI-binding generator, and `tools/New-OpInventory` — which conflicts with the standing ruling that `inventory.gen.go` is hand-edited and that tool never run. Its copy of the rename plan is OLDER than develop's (it still says `Operation`, not `Step`), so nothing to take there. Each item needs a promote-or-discard verdict; the worktree is removed only after promotions merge, and before step 56's rename window opens. | Step doc: [phase-8/steps/57-mine-the-workflow-rename-worktree.md](phase-8/steps/57-mine-the-workflow-rename-worktree.md) |
| 58 | The System target root is `/` on every platform, which on Windows means the current drive | **[#392](https://github.com/NobleFactor/devlore-cli/issues/392)** · **charter 2026-08-16** — [`layer.go:29`](../../../cmd/writ/writ/layer.go) is the only place the System root is decided, and it is the literal `/`. Go treats a leading separator with no volume as **drive-relative**, so a System deploy lands on whatever drive the operator was standing on — the same defect family as step 54, one level up: relative to the working *volume* rather than the working *directory*. The docs compound it, describing the scope as `%SystemRoot%` (`C:\Windows`, a subdirectory) in two places no code reads. Exposure is currently **zero** — no test deploys through the System scope and the deploy scenario has no `System/` fixture — which is why it survived and why the fix is cheap. **The windows campaign is not done until this closes.** | Step doc: [phase-8/steps/58-windows-system-target-root.md](phase-8/steps/58-windows-system-target-root.md) |
| 59 | `ConfigDirs` and `DataDirs` return Unix paths on every platform | **[#437](https://github.com/NobleFactor/devlore-cli/issues/437)** · **charter 2026-08-16** — [`xdg.go:121`](../../../pkg/xdg/xdg.go) and [`:142`](../../../pkg/xdg/xdg.go) return the specification's defaults `/etc/xdg` and `/usr/local/share:/usr/share`, which are correct on Linux and Darwin and wrong twice on Windows: the directories do not exist, and a leading separator with no volume is **drive-relative** — step 58's defect one level out. The package documents the invariant they break ("no anchor can resolve to a relative path"); `TestNoAnchorIsEverRelative` proves it for the five **base** accessors and does not cover these two, which is the gap that let it ship. Exposure is **zero** — no callers outside the package's own tests — and the fix is cheap now and inherited later: a system-wide install makes `DataDirs()` the natural place to look for machine-wide completions and man pages. Windows has no search-path concept to copy, so the semantics need researching the way step 54's layout ruling was. | Step doc: [phase-8/steps/59-xdg-search-paths-on-windows.md](phase-8/steps/59-xdg-search-paths-on-windows.md) |
| 60 | The store writes `latest.yaml` as a symlink an ordinary Windows user cannot create, and nothing reads it | **[#438](https://github.com/NobleFactor/devlore-cli/issues/438)** · **charter 2026-08-17** — opened by asking whether phase 3.3a needed per-platform save/load scenarios; it does not, but three findings sit behind the question. (1) [`store.go:141`](../../../cmd/internal/cli/store.go) creates `latest.yaml` with `Symlink`, which on Windows needs `SeCreateSymbolicLinkPrivilege` or Developer Mode — an ordinary user has neither, so every `WriteTrace` errors there. All nine call sites **warn rather than fail**, so the trace lands on disk, the command reports success, and `appendIndexEntry` — which sits **after** the link — never runs: the run index silently loses every trace on that machine. That ordering is a defect on every platform. (2) `LoadLatestTrace` is the link's only reader and has **no callers at all**, so the store fails a write step for an entry point nothing uses — decide whether it earns its place before deciding how to spell it. (3) Cross-platform document identity is unproven, and a scenario is the wrong instrument (each runs on one platform and compares against itself); the instrument is a committed graph + trace fixture loaded by every leg. **CI cannot see any of this** — the Actions Windows runner holds the symlink privilege, so the proof run must DENY it, the same lesson phase 2 learned by testing the safest root instead of the one production uses. | Step doc: [phase-8/steps/60-execution-store-cross-platform.md](phase-8/steps/60-execution-store-cross-platform.md) |
...
- `options.Label` supplied → registered under that label; auto-label
  skipped.
- `options.Label` empty → auto-labeled `<provider>.<method>#<N>` where
...
## Closure pass — queued behind the windows campaign (2026-08-14)

Phase 8 is nearly closable and the ledger overstates what is left. Of 54 steps, 50 are marked complete; 23 of
those carry a remainder in their notes, but the remainders are three different things and only one is work:

1. **Stale text.** Steps 15, 24, 26 and 40 record their own closure inside the note that still reads as open.
   Rewording is the whole job.
2. **Owned elsewhere.** Step 18's build reds are step 33's by explicit assignment; step 42's receipt alignment
   became step 44; step 36's provisional events wait on step 50. Cross-references, not debt.
3. **Genuinely open and small.** Step 12's three matrix rows, step 13's checksum verification (#282), step 25's
   two conversions plus the home-of-record decision, step 22's k.13 lifecycle, step 19's `cmd/` consumer
   migration, step 49's encryption write path bypassing the conflict seam.

Separately, four items are **not phase-8 work any more** and should be re-chartered as their own plans rather
than keeping a fifty-step ledger open behind them: step 38 (elevation policy, unstarted design), step 50
(eventing implementation — it has its own architecture doc and five settled decisions), steps 34/39/51 (design
and documentation debt), and step 46's custody deferrals (agent/KMS, cert authority).
