...
| 33 | `writ migrate` — full rewrite onto the sealed-graph executor | in-progress — slices A (adopt) + B (migrate) LANDED 2026-07-15 (adopt: gather+projection batches; migrate: one-run restructure + two-run RegisterLayer on the common-ancestor root, session/batch converged, file_ops.go deleted); next: slice D (verify + close); C (the deploy-family StateView crater) lies until A/B/D land | **Added 2026-06-19. Full rewrite, not an incremental fix.** `migrate/Execute` hand-executes the graph (N one-node graphs via slot strip-mining) instead of running it; collapse onto `GraphExecutor.Run` per the session.go pattern. Owns step 18's remaining build reds (writ family + docgen + e2e). Charter + red inventory: [phase-8/steps/33-writ-migrate-rewrite.md](phase-8/steps/33-writ-migrate-rewrite.md). |
