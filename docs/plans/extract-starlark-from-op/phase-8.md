...
| 39 | Complete per-provider design docs | in-progress — **12 of 14 landed**: 3.5.3 plan (2026-07-21), 3.5.4 file, batch 1 (3.5.5 json / 3.5.6 yaml / 3.5.7 template), batch 2 (3.5.8 shell / 3.5.9 powershell — the catalog's only zero-coverage provider → step-52 rows 11–12), batch 3 (3.5.10 git — dirty-aware Digest/Etag content identity, clone-only compensation; Checkout/Pull/Observe gaps → step-52 rows 14–15; 3.5.11 service — prior-state receipts, `CompensateRestart` no-op ruling), batch 4 (3.5.12 appnet — identity is the address, not the content; 3.5.13 encryption — SOPS pairs over the file taxonomy; 3.5.14 function — synthesized source is identity, bytecode is cache, version-skew recompile; gaps → step-52 rows 16–18) — each with greped test matrices, catalog rows, index entries; last: regexp/ui | 14 of 18 providers have no design doc; one 3.5.x doc + .status.md companion each, on the 3.5.2 pattern (every method covered, API surface, dual-API usage, verified per-method test matrix). Priority: plan (3.5.3), file (3.5.4), then the remaining planned/both providers. Charter: [phase-8/steps/39-per-provider-design-docs.md](phase-8/steps/39-per-provider-design-docs.md). |
