...
| 13 | plan.choose initial redesign (superseded by 13b) | complete, superseded | Committed at 22f43d7. `flow.Provider.Case` redefined as `Case{When any, Then any}` pure data; `flow.Provider.Choose` as compensable `(any, op.Complement, error)` with a `CompensateChoose` companion; `flow/helpers.go` adds `isTruthy`; `plan.Provider.Case` added on plan-Tier 1. Design review rejected the compensable-triple shape: (a) side-effecting Whens execute regardless of case selection because evaluating the When *is* running it; (b) per-method compensation doesn't model per-branch activation; (c) control-flow semantics (short-circuit, per-iteration scope, polling) belong in graph topology rather than as one-off method bodies. Superseded by 13b which recasts choose / gather / subgraph / wait_until as subgraph kinds with kind-specific executors. |
| 13b.1 | PlanM companion pattern + rename `<M>Planned` → `Plan<M>` | not-started | **Unify all plan-time companions under the `Plan<M>` prefix**. Today `<M>Planned` (suffix) denotes the pure plan-time resource-identity companion (`WriteTextPlanned` for `WriteText`, `CopyPlanned` for `Copy`, etc.). The user never authorized the suffix convention; it is retired in this step in favor of the `Plan<M>` prefix, which also serves the new plan-time-builder companions landing in 13b.2. Same machinery, unified naming. **Rename**: every `<M>Planned` method across providers → `Plan<M>` — 15 sites across file (`Copy`, `Link`, `Move`, `WriteBytes`, `WriteText`, `Mkdir`), git (`Clone`), service (`Disable`, `Enable`, `Restart`, `Start`, `Stop`), archive (`Extract`), encryption (`DecryptSopsFile`). **Codegen (`star/extensions/com.noblefactor.devlore.Actions/commands/generate.star`)**: `filter_methods` drops the `m.name.endswith("Planned")` exclusion and adds a `m.name.startswith("Plan") and <strip_and_check_primary_exists>` exclusion (the primary method must exist for the `PlanM` to count as a companion; avoids collision with hypothetical independent `Plan*` methods). Doc comment updated. **Reflection lookup (`pkg/op/receiver_type.go:543`)**: `providerType.MethodByName(method.Name + "Planned")` becomes `providerType.MethodByName("Plan" + method.Name)`. **Accessors on `pkg/op/method.go`**: `Method.planned` field, `HasPlanned()` / `Plan()` method names retained (still semantically accurate — they refer to the Plan companion). Only the prefix-vs-suffix detection changes. **Documentation**: `docs/architecture/4-resource-management.md:872-890` "companion triplet" section updated to reflect `Plan<M>` instead of `<M>Planned`; source-order guidance becomes "forward, plan, compensate" (i.e., `Copy`, `PlanCopy`, `CompensateCopy`). **Regenerate**: every `*/gen/*.go` file that references the changed method names (flow, file, git, service, archive, encryption). **Tests**: rename test function `TestWalkTreePlanned` → `TestWalkTreePlan` (or similar), update `test_walk_tree_planned.star` filename. `cmd/devlore-test/devloretest/data/test_walk_tree_planned.star` → `test_walk_tree_plan.star`. **Verification**: `make build` clean; full test suite passes; all existing resource-identity plan-time behavior preserved (only the names changed). **Non-goals**: no semantic changes; no new methods added; no conditional edges, no Case/Default, no activation executor — those all land in 13b.2 on top of the renamed foundation. |
| 13b.2 | Conditional edges + Case/Default descriptors + activation-aware executor + Halt + Choose/PlanChoose + Subgraph recast | not-started | Builds on 13b.1's `Plan<M>` prefix convention. Three method shapes result for Providers — (a) `M` + `PlanM` (primary + Plan companion): plan-time builds an artifact, execute-time invokes `M` on that artifact (e.g., `Choose` + `PlanChoose` here; `Gather` + `PlanGather` in 13c; `WaitUntil` + `PlanWaitUntil` in 13d; resource-producers like `WriteText` + `PlanWriteText` already renamed in 13b.1); (b) `PlanM` only, no primary: plan-time builds a descriptor or KindNormal subgraph; no per-kind execute-time code (`PlanCase`, `PlanDefault`, `PlanSubgraph`); (c) `M` only, no Plan companion: default NodeBuilder path (today's behavior — `Complete`, `Degraded`, `Fatal`). **pkg/op/ control-flow primitives**: `edge.go` (new) defines `EdgeCondition` interface (`Evaluate(sourceOutput any) bool` + `Kind() string` for serialization) with package-level singletons `TruthyCondition{}` / `FalsyCondition{}`; `Edge` gains `Condition EdgeCondition` field (nil = unconditional; preserves today's semantics for non-choose subgraphs). `case.go` (new) defines `Case{When, Then op.ExecutableUnit}`, `Default{Body op.ExecutableUnit}`, and `Branch` interface with unexported marker `isBranch()` satisfied by `*Case` and `*Default`. These descriptors are **not** `op.ExecutableUnit`s — they flow only into `flow.Provider.PlanChoose` at the Go type level, which enforces "plan.case cannot appear outside plan.choose" via the type system. `halt.go` (new) defines `Halt{Value any, Status SubgraphStatus, Err error}`. `subgraph.go` gains `SubgraphKind` (`KindNormal=""`, `KindChoose`, `KindGather`, `KindWaitUntil` — Case/Default are descriptors, not kinds), `Executor SubgraphExecutor` (`json:"-" yaml:"-"`; set by PlanM to the Provider instance for kind-bearing subgraphs), `Properties map[string]any`; `Branch bool` removed. `subgraph_executor.go` (new): `SubgraphExecutor` interface (`Execute(d Dispatcher, sg *Subgraph, overrides map[string]SlotValue) (any, error)`) and `Dispatcher` interface (`Dispatch(unit ExecutableUnit, overrides) (any, error)` + `WithStack(stack *RecoveryStack) Dispatcher`). **pkg/op/graph.go + executor.go**: `dispatch` routes `case *Subgraph` through `sg.Executor` when non-nil; unexported `dispatcher` adapter captures `(ctx, e, g, stack, results)`. `executeChildren` / `executeSubgraph` (the KindNormal default executor) rewritten around two layered contracts — (1) **activation gate**: maintain `activated map[string]bool` seeded with topological roots; after each child dispatch, evaluate every outgoing edge's `Condition.Evaluate(result)` (or unconditional = always true) and mark target activated only when condition holds; skip non-activated children. (2) **halt-or-[]any**: a child returning `op.Halt` sets `subgraph.Status = halt.Status` and short-circuits; `SubgraphFailed` propagates `halt.Err`. Otherwise the subgraph result is `[]any{result(child_1), ..., result(child_n)}` in topological order (activated children only) — replaces today's `lastResult` scalar (`executor.go:282-286`). **pkg/op/provider/flow/provider.go**: `Case` struct removed. `Complete` / `Degraded` / `Fatal` now return `op.Halt` (execute-time terminal signals). Old `Choose` / `CompensateChoose` / old `Subgraph` stub removed. New primary `Choose(d op.Dispatcher, sg *op.Subgraph, overrides map[string]op.SlotValue) (any, error)` — walks activated branches via conditional-edge traversal + isTruthy evaluation, returns the activated branch's value. New Plan companions: `PlanChoose(branches ...op.Branch) *op.Subgraph` (constructs KindChoose subgraph with `sg.Executor = p`; wires conditional-edge topology — TruthyCondition When→Then per case, FalsyCondition chain between consecutive Whens, FalsyCondition from last When to trailing Default). `PlanCase(when, then any) *op.Case` and `PlanDefault(body any) *op.Default` construct descriptors (no primary methods). `PlanSubgraph(children ...op.ExecutableUnit) *op.Subgraph` constructs a KindNormal subgraph (no primary; default executor handles it). `helpers.go` `isTruthy` retained (used by `TruthyCondition.Evaluate` and by primary `Choose`); `helpers_test.go` rewritten to cover activation-based choose execution with synthetic topologies. **pkg/op/provider/plan/**: `Case` method dropped; `flow.Provider.PlanCase` surfaces as `plan.case` via Tier 2 (flow is root=true). Tier 2 peer discovery extended to recognize PlanM-routed methods and install the generated plan-time dispatch wrappers. **Regenerate**: `flow/gen/provider.gen.go` (announce map gains Choose/PlanChoose/PlanCase/PlanDefault/PlanSubgraph + generated `Provider.Execute`); `flow/gen/receiver_type.gen_test.go`; `plan/gen/*.go`. **Tests**: new `op/executor_activation_test.go` (activation semantics); new `op/executor_halt_test.go` (halt-or-[]any contract); `flow/helpers_test.go` rewritten. Integration smoke: `plan.choose(plan.case(true, "yes"), plan.default("no"))` → "yes"; `plan.choose(plan.case(false, "no"), plan.default("yes"))` → "yes"; `plan.subgraph(a, b, c)` with distinct literals → `[]any{a, b, c}`; `plan.subgraph(a, plan.complete("early"), c)` → "early". **Compensation shift**: Choose loses its `(T, Complement, error)` triple and `CompensateChoose` companion; per-branch complements flow through the dispatcher's recovery stack automatically — activation ensures only executed nodes push complements. |
| 13c | Gather recast (Gather + PlanGather pair) as KindGather subgraph | not-started | Two methods on `flow.Provider`: `PlanGather(items any, body any, limit int) *op.Subgraph` (plan-time companion — constructs KindGather subgraph with `sg.Executor = p`; items/limit stored in `sg.Properties`; body as `Children[0]`) and primary `Gather(d op.Dispatcher, sg *op.Subgraph, overrides) (any, error)` (execute-time — invoked by generated `Provider.Execute` when `sg.Kind == KindGather`). The Phase-7 goroutine orchestration (current `flow/provider.go:196-298`) lifts wholesale into `Gather`'s body; each iteration scoped via `d.WithStack(iterStack)` so compensation is per-iteration. On total success, per-iteration stacks are pushed onto the parent stack (compensation unwinds all iterations in reverse completion order) — equivalent to the current CompensateGather behavior but without a separate method. On any iteration failure: cancel scoped ctx, wait, unwind locally-held stacks, propagate error. `CompensateGather` method removed. Starlark: `plan.gather(items=..., body=..., limit=...)` surfaces via Tier 2 (flow is root=true). Regenerate `flow/gen/provider.gen.go`. Tests cover total success, iteration failure → cancel-and-unwind, empty items, concurrency-limit respected. |
| 13d | WaitUntil recast (WaitUntil + PlanWaitUntil pair) as KindWaitUntil subgraph | not-started | Two methods on `flow.Provider`: `PlanWaitUntil(target any, predicate any, timeout, interval time.Duration) *op.Subgraph` (plan-time — constructs KindWaitUntil subgraph with `sg.Executor = p`; target/timeout/interval stored in `sg.Properties`; predicate as `Children[0]`) and primary `WaitUntil(d op.Dispatcher, sg *op.Subgraph, overrides) (any, error)` (execute-time — invoked when `sg.Kind == KindWaitUntil`). The polling loop (current `flow/provider.go:361-403`) lifts into `WaitUntil`'s body — each iteration dispatches the predicate invocation via `d.Dispatch(predicateChild, overrides)` and applies `isTruthy` to the result (not a native Go `func(any) (bool, error)` as today). Starlark: `plan.wait_until(target=..., predicate=..., timeout=..., interval=...)` surfaces via Tier 2. Regenerate `flow/gen/provider.gen.go`. Tests cover immediate match, timeout, predicate error, per-tick dispatch. |
| 14 | plan.gather redesign | superseded by 13c | — |
| 15 | plan.wait_until redesign | superseded by 13d | — |
...
**Status:** in-progress. Steps 1–13 complete; step 13's Choose design
was superseded in review. The redesign unifies all plan-time
companions under a **`Plan<M>` prefix** (peer to the existing
`Compensate<M>` prefix). The unauthorized `<M>Planned` suffix — today
used for pure plan-time resource-identity companions like
`WriteTextPlanned`, `CopyPlanned` — is retired and migrated to the
prefix form (13b.1). Three method shapes result for any Provider —
`M` + `Plan<M>` (primary + Plan companion: plan-time builds an
artifact, execute-time invokes `M` on that artifact; resource
producers like `WriteText` + `PlanWriteText`, control-flow kinds
like `Choose` + `PlanChoose` / `Gather` + `PlanGather` / `WaitUntil`
+ `PlanWaitUntil`); `Plan<M>` only, no primary (builds a descriptor
or KindNormal subgraph: `PlanCase`, `PlanDefault`, `PlanSubgraph`);
or `M` only, no Plan companion (today's default NodeBuilder
dispatch: `Complete`, `Degraded`, `Fatal`). No separate Builder
type — everything stays on the existing Provider. Control flow uses
**conditional edges** as the topology primitive: `Edge` gains a
`Condition` field (`TruthyCondition` / `FalsyCondition` singletons);
`PlanChoose` wires the conditional topology from `*op.Case` /
`*op.Default` descriptors; primary `Choose` walks the resulting
subgraph with activation-driven execution at execute time. The
default KindNormal executor also gains activation + halt-or-[]any
semantics. **13b.1** lands the PlanM codegen pattern and renames
every existing `<M>Planned` → `Plan<M>` — mechanical, no semantic
change. **13b.2** adds conditional edges, Case/Default descriptors,
the activation executor, op.Halt, and the Choose/PlanChoose +
PlanCase/PlanDefault/PlanSubgraph methods on `flow.Provider`;
`Complete`/`Degraded`/`Fatal` migrate to `op.Halt` returns. **13c**
adds `Gather` + `PlanGather` (KindGather with `Dispatcher.WithStack`
for per-iteration scopes). **13d** adds `WaitUntil` + `PlanWaitUntil`.
Compensation shifts from per-method (`CompensateX` companions) to
dispatcher-stack-level — activation ensures only executed nodes push
complements; iteration kinds (13c) assemble per-iteration stacks into
a parent-level entry. 16–21 unchanged; D1–D12 resolved; D13
(marshaling) pending on O1's five questions.
