...
| 25 | `cmd/star` lint-copyright starlark resolution bug (Row-4) | not-started | Single root cause behind all 9 `TestLintCopyright_*` cases + `TestSourceFile_StarlarkIntegration`: the lint-copyright command script reads `cfg.lint.copyright` (`lint-copyright.star:304`), but `cfg` resolves to a `builtin_function_or_method`, so `.lint` errors with `builtin_function_or_method has no .lint field or method`. A config/namespace-injection issue in `cmd/star`'s starlark surface — isolated from the framework and the lore path. Surfaced by the 2026-05-31 QA report (finding F3). **Must be resolved (or deliberately re-justified) before phase-8 closes** — do not move on from phase 8 with this red. |
