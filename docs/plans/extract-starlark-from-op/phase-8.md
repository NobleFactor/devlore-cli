...
| 12.0(h) | Post-13.0(b) cleanup | complete | `wrapper`→`goReceiver` rename; `file.Receipt.recoveryID` field; stack-comparison docs refreshed; bare-UUID test sweep (9 sites); `ui.Provider` test pull. |
...
**Status:** in-progress (Phase 6 of 12.0(n) closed 2026-05-24; phase-8 exit pending steps 13, 14, 20–25 + O1/O2/O3).
Steps 1–11 complete. **12.0 (a) through (n) all closed.** The step-20 graph-immutability seal landed in
