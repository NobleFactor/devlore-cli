...
| 46 | Graph signing + `writ verify` — build `pkg/signing`, settle the scheme, ship the command | design CLOSED 2026-07-16 — questions 1–3 were already settled by the design docs (graph-signing.md: sign namespaced canonical bytes, graphs AND traces, inline op.Signature, no envelope; signing-options.md: ssh-ed25519 default via the developer's SSH key, allowed_signers trust, sigstore only for opt-in KMS/keyless); question 4 ruled: signing.policy ∈ {ignore, report, reject_external, reject}, floor report, store-boundary externality. Implementation next (the default tier) | **Added 2026-07-15.** Nothing signs graphs today: no `op.Signature` is constructed anywhere; `op.NewGraph` defers to a `pkg/signing` that does not exist; the sealed scheme declaration (ed25519/ecdsa-p256) conflicts with the inherited helper's age expectation. Scope: the framework signer, the scheme ruling, `writ verify <graph-document>...`, reconcile integration. Four design questions (scheme; graphs-only or traces too; identity/key source; `unsigned` severity) settle one at a time before code. Step doc: [phase-8/steps/46-graph-signing-and-verify.md](phase-8/steps/46-graph-signing-and-verify.md) |
