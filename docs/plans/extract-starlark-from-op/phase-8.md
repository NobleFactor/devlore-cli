...
| 39 | Complete per-provider design docs | in-progress — **3.5.3 plan landed 2026-07-21** (the priority-1 doc: three-tier surface, detached invocations, dual door, assembly, lifecycle; status companion carries the greped per-method test matrix); 13 providers remain, file (3.5.4) next | 14 of 18 providers have no design doc; one 3.5.x doc + .status.md companion each, on the 3.5.2 pattern (every method covered, API surface, dual-API usage, verified per-method test matrix). Priority: plan (3.5.3), file (3.5.4), then the remaining planned/both providers. Charter: [phase-8/steps/39-per-provider-design-docs.md](phase-8/steps/39-per-provider-design-docs.md). |
