...
| 13.0(i) | `ui.Provider` → `workflow.UI` capability migration | not-started | Replaces `ui.Provider` with a runtime-environment capability the way `platform.Provider` already does. Four-step skeleton: **(1)** Migrate `ui.Provider` implementation to a `workflow.UI` interface/instance — UI capability lives outside the provider boundary. **(2)** `RuntimeEnvironmentSpec` holds a `UI` instance; `RuntimeEnvironment` holds the same `UI` instance (config-time and runtime symmetry, mirroring the platform pattern). **(3)** `ui.Provider` becomes a thin starlark-facing wrapper that forwards to `RuntimeEnvironment.UI`. **(4)** Every provider that needs UI services (currently reaching into `internal/cli` `statusOutput` or constructing `&ui.Provider{}` directly) calls `RuntimeEnvironment.UI` methods instead. Cleans up the KNOWN BREAK from 13.0(b): `cmd/star/cli/output.go:358-360`, `cmd/star/star/application.go:58-60`, `internal/cli/output.go:108-110/115/120/125` all stop constructing `&ui.Provider{Writer/ProgramName/Color/Silent: ...}` literals. Design refinement pending — this row captures the agreed direction at high level; specific surface area, ownership of `Writer`/`ProgramName`/`Color`/`Silent` semantics, and the lifecycle of `statusOutput` are open questions for the design pass. |
