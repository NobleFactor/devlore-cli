...
| 34 | `RecoveryStack.Unwind` takes the runtime environment; `Push`/`PushNested` drop it | not-started (added 2026-06-27) — design settled | **Added 2026-06-27. Final step — sequenced after the complement-shape / flow.gather work, since it touches every `Push`/`Unwind` call site.** `RecoveryStack.Unwind` gains a `*op.RuntimeEnvironment` parameter and passes it to each entry's compensation; `RecoveryStack.Push` and `RecoveryStack.PushNested` **drop** the env parameter. Today `Push` captures the env solely to pre-bind `invokeCompensateForReceipt` (`recovery_stack.go:111`), the binding is never serialized, and `rearm` re-binds it with the resume env — so the env-at-`Push` is redundant with the resume path. Deferring it to `Unwind` makes a single bind site: `Push` records the receipt, `Unwind` supplies the env at compensation time, and `rearm` no longer re-captures it. Resolves step-28's open design question 3. |
