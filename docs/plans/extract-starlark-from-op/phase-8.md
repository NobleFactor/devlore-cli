...
| 21 | Test triage — pre-existing failures | not-started | Resolve the suite of pre-existing red tests that predate phase-8's binding-model work: `TestImm*` (9 tests; immediate-mode .star scripts that assert `unit_count(0)` — likely script-side drift post-13.0(n)); `TestWalkTreePlanned` (1 test; `*starlark.Function value is neither assignable nor convertible to file.Reducer` — callable-typed slot parameter not handled by the bridge's converter cascade); `TestCLI_GraphOnly` + `TestCLI_RoutToFiles` (2 tests in `cmd/devlore-test`; subprocess CLI tests, diagnosis pending); `TestLintCopyright_*` (8 subtests; copyright lint check). Also: resolve the `starlarkbridge.NewProvider` / `ReceiverName` template staleness flagged during step 2 (module test template references APIs removed during Phase 7/8 refactoring). All 21 reds → green is non-negotiable for phase-8 PR per the step-23 gate. |
...
- **Step 23 — Framework helper direct-test backfill + PR gate.** Phase 6.0's convertibility-layer
  ...
  plumbing.
- **Step 21 — test triage.** 11 pre-existing red tests must turn green for the PR gate: `TestImm*` (9),
  `TestWalkTreePlanned` (1), `TestCLI_*` (2), `TestLintCopyright_*` (8 subtests).
