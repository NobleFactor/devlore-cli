...
**Remaining within 13.0(k) — two items:** k.14 caller-migration sweep (file-provider bug list orphaned
during 13.0(e); scope unknown until `pkg/op/provider/file` is audited); k.15 reconciliation pass (walk
catalog entries at session start, run the Etag/Digest cascade as side effects — this is where k.10's drift
signal becomes actionable). **(b)** Successor
