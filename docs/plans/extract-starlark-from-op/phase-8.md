...
(two-constructor pattern, producer-stamp verification, executor post-dispatch block deleted). 13.0(n) writ
graph executor is the only remaining not-started item outside 13.0(k) — 8 nil-activation callsites in
`cmd/writ` block strict activation enforcement. The k.12 boot-discipline test landed at
...
**Remaining within 13.0(k) — one item:** k.15 — add `(*ResourceCatalog).ResolvePending() []error`.
Catalog-owned sweep that walks every Pending entry in URI order, releases the mutex during each
`r.Resolve()` call, applies `markActive`/`markGone` per outcome, and returns aggregated failures.
...
k.15, moved out of 13.0 — tracked as #282 under step 16's preflight scope. k.14 closed by audit: every
