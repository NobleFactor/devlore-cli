...
| 33 | `writ migrate` — full rewrite onto the sealed-graph executor | in-progress — DESIGN PHASE (2026-07-15): per-command plan & design docs drafted (adopt/migrate/verify); adopt design settled (gather + field projection, all questions ruled); slice order 45 → A → B → D → C (C lies until A/B/D land); slice-1 API groundwork in-tree | **Added 2026-06-19. Full rewrite, not an incremental fix.** `migrate/Execute` hand-executes the graph (N one-node graphs via slot strip-mining) instead of running it; collapse onto `GraphExecutor.Run` per the session.go pattern. Owns step 18's remaining build reds (writ family + docgen + e2e). Charter + red inventory: [phase-8/steps/33-writ-migrate-rewrite.md](phase-8/steps/33-writ-migrate-rewrite.md). |
