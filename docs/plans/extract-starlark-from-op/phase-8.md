...
| 39 | Complete per-provider design docs | in-progress — **3.5.3 plan (2026-07-21) + 3.5.4 file (2026-07-22) landed** (plan: three-tier surface, detached invocations, dual door, assembly, lifecycle; file: sealed taxonomy, unified mutation receipt with five kinds + one compensating action, conflict-policy write seam; both status companions carry greped per-method test matrices — file's spans 141 Go tests + 18 fixtures); 12 providers remain | 14 of 18 providers have no design doc; one 3.5.x doc + .status.md companion each, on the 3.5.2 pattern (every method covered, API surface, dual-API usage, verified per-method test matrix). Priority: plan (3.5.3), file (3.5.4), then the remaining planned/both providers. Charter: [phase-8/steps/39-per-provider-design-docs.md](phase-8/steps/39-per-provider-design-docs.md). |
