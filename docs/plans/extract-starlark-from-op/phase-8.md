...
| 13.0(i) | UI capability split: `pkg/status` (side channel) + `pkg/result` (primary output) on the runtime environment | not-started | **Design settled 2026-05-01.** Replaces `ui.Provider`'s embedded state and the `internal/cli` `statusOutput` package global with two complementary capabilities owned by the runtime environment, following the `platform.Provider` precedent. **Two packages, sibling to `pkg/assert` and `pkg/iox`:** **`pkg/status`** owns the side channel — interface `status.UI` with methods `Note(format string, args ...any)`, `Warn(...)`, `Error(...)`, `Success(...)`, `Fail(...) error`. Default impls: `status.Console` (TTY-aware: `io.Writer` + program name + color flag) and `status.NoOp` (silent/test). Maps to stderr by Unix convention. Name chosen over `notify` to dodge fsnotify-cloud collisions in the Go ecosystem (per prior-art research: fsnotify, rjeczalik/notify, nikoksr/notify, dbus/notify all dominate `notify` namespace; `status` is unambiguous and matches kubectl/gh `ErrOut` semantics). **`pkg/result`** owns the primary output — interface `result.Sink` with `Emit(value any) error`, composed with two pipeline-stage interfaces: `result.Filter.Apply(value any) (any, error)` (jq-style projection / no-op / schema-based selection) and `result.Formatter.Format(value any, w io.Writer) error`. Default `result.NewPipeline(filter, formatter, w)` returns the canonical `Sink`. Three formatters ship: `result.JSONFormatter`, `result.YAMLFormatter`, `result.CSVFormatter`. **Tabular = CSV decision.** The "table" format slot binds to CSV (RFC 4180): machine-honest bytes with stable downstream contract for diffs/grep/spreadsheets/csvkit/duckdb; padding-shifting aligned columns are a *presentation* concern layered separately, not a competing format. A future `--pretty` toggle (or auto-detected TTY-on-stdout) can wrap CSV in a column-alignment pass without touching the formatter set. CSV schema inference: reflect first-row struct fields by default; opt-in `Headers() []string` interface for explicit control; for `[]map[string]any` use union of keys, sorted. Pipeline shape: `any → structured document → result.Filter.Apply → result.Formatter.Format → io.Writer`. **Capability injection.** `op.RuntimeEnvironmentSpec` and `op.RuntimeEnvironment` each gain `Status status.UI` and `Result result.Sink` fields. Spec hands the *same instances* to the environment at materialization — no copy, no rebuild. Same symmetry rule as `platform.Provider`. Defaults when fields are zero: `status.NoOp{}` (silent) and a `result.Sink` whose `Emit` returns an "unconfigured result sink" error (loud failure rather than silent swallow). **Bootstrap solved by separating construction from injection.** Each client (lore, star, writ) constructs its own `status.UI` and `result.Sink` early — usable immediately for flag-parse and config-load errors. The same instances flow into `RuntimeEnvironmentSpec` when the environment is built later. No separate "early" vs. "late" code path; identity carries through. **`ui.Provider` shape post-migration.** Becomes a thin starlark adapter — method bodies forward to `ctx.Environment.Status.<Note|Warn|Error|Success|Fail>(...)`. No state, no embedded writer/programName/color/silent. The package may eventually be renamed (e.g., `status.Provider`), but renaming is cosmetic and orthogonal to this migration. **Caller migration sites.** Closes the KNOWN BREAK from 13.0(b): `cmd/star/cli/output.go:358-360`, `cmd/star/star/application.go:58-60`, `internal/cli/output.go:108-110/115/120/125`. The `internal/cli/statusOutput` package global goes away — bootstrap output uses a `status.Console` constructed against `os.Stderr` directly, then the same instance is handed to `RuntimeEnvironmentSpec`. Result-emission sites currently building JSON/YAML/table formatters inline rewire through `env.Result.Emit(value)`. Existing `internal/output.Options` (format + filter flags) translates into `result.Filter` + `result.Formatter` selection at command-setup time. **Open implementation questions deferred to the implementation pass:** (a) `result.Sink.Emit` signature — `any` (flexible, formatter does inference) vs. constrained interface (`MarshalJSON`/`MarshalYAML`/`Headers()`-style). Default to `any`. (b) Formatter package layout — flat (`result.JSONFormatter` etc.) vs. subpackage (`pkg/result/format/json` etc.) to keep yaml.v3/csv deps out of every `result.Sink` import. (c) Default jq-style filter — ship gojq dependency baked in (gh model) or leave wiring to the client. **Prior art landed on:** kubectl `IOStreams` + `printers.ResourcePrinter`, gh `iostreams` + `json_printer` with `--jq`/`--template`, AWS CLI `--query`/`--output`, Terraform `tfdiags` + `views`. The `Sink` / `Filter` / `Formatter` triple isn't a single recognized pattern but each piece is established (`zap.Sink`, `logrus.Formatter`, `slog.Handler`). |
