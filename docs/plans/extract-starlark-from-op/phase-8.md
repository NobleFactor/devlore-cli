...
| 10 | starlarkbridge.Invocation as starlark.Value; dispatch returns `*Invocation` | not-started | Add `Freeze`/`Hash`/`String`/`Truth`/`Type` and Promise-compatible `Attr`/`AttrNames` to `*starlarkbridge.Invocation` so every callsite that consumed `*starlarkbridge.Promise` continues to work. `NodeBuilder.dispatch` return type changes from `*starlarkbridge.Promise` to `*starlarkbridge.Invocation`; Promise becomes an internal helper. |
...
**Status:** in-progress. Steps 1–9 complete; 10–21 pending. Step 9
aligned NodeBuilder with D5's detached-invocation model: no graph
reference at dispatch time, edges implicit in slot PromiseValues +
Resource originIDs, catalog and ctx wired directly. plan.Provider
no longer constructs an op.Graph — that materializes at plan.run
(step 16) from the reachable invocation set. Step 10 is next —
promoting `*Invocation` to `starlark.Value` and switching
dispatch's return type from `*Promise` to `*Invocation`. Design
decisions D1–D12 are resolved; D13 (marshaling) is pending and
will be written once O1's five questions are answered.
