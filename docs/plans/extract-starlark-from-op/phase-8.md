...
| 13.0(e) | Saga shape and stack-based recovery | in-progress | Steps 1–5b complete (Action.FullName, RecoveryStack API, Method classifier + Invoke + pushComplement dispatcher, Tombstone→Receipt rename). Complement shape settled to `*op.RecoveryStack` as the singular legal form. Closure-only `Push`/`Do` APIs already deleted from `RecoveryStack` (ahead of plan). **Remaining: file-provider bug list (k.14).** The list itself is not enumerated in this plan — it was orphaned during the closure-only-API deletion when call sites in `pkg/op/provider/file` were left referring to APIs that no longer exist. Closing k.14 requires first auditing `pkg/op/provider/file` for any caller that still expects the old Push/Do shape (or other RecoveryStack closure-only surface) and migrating each to the dispatcher form. The audit is itself part of k.14's scope; treat the bug list as "unknown until audited," not "small." |
...
| 13.0(g) | Resource construction at the bridge boundary | in-progress | Landed: wrapper/Projector/`op.Convert` refactor; `op.SourceConverter`/`op.TargetConverter` split; producer rename (`originID`→`producerID`); catalog state model documented. Remaining: lifecycle integration items (c)–(f) (Created→Active→Gone states, markActive, provider deletion paths, executor Shadow gating) absorbed into 13.0(k) k.13. |
...
cascade outside the catalog mutex. **Remaining within 13.0(k) — three items, not two:** k.13 lifecycle
integration (Created → Active → Gone state, `markActive` after construction, provider deletion paths, and
executor Shadow gating — bigger than a state enum); k.14 caller-migration sweep (file-provider bug list
orphaned during 13.0(e); scope unknown until `pkg/op/provider/file` is audited); k.15 reconciliation pass
(walk catalog entries at session start, run the Etag/Digest cascade as side effects — this is where k.10's
drift signal becomes actionable). **(b)** Successor
