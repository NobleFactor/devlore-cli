...
| 4  | **Planned-mode graph assembly** — `plan.assemble`, orphan detection, plan-time type-check | phase-8 steps 13 / 14 / 16 | ✅ |
| 5  | **Sealed immutable Graph API** — re-executable plan; run-state off the graph; **lore/writ consumer migration complete** — **gates Scenario 1** | the graph-immutability work + [graph-immutability.md](phase-8/graph-immutability.md) | ⬜ |
...
| 15 | **Function values through the bridge** — starlark callbacks → typed Go funcs | phase-8 step 25 (`TestWalkTreePlanned`) | ⬜ |
| 16 | **Full `make test` green** — phase-8 PR gate | phase-8 step 24 | ⬜ |
...
end-to-end** (see [phase-8/graph-immutability.md](phase-8/graph-immutability.md)). Enabling work layered on the seal:
...
and the full suite is green (criterion 16). The graph-immutability seal is
green in `pkg/op` but its `lore`/`writ` consumer migration is in flight, which is
why criterion 5 is ⬜.

## Relationship to phase-8

Most criteria are phase-8 steps; criterion 16 *is* the phase-8 PR gate (step 24).
