...
The boundary `rootDir` is our own **`fsroot.Dir`** — the provider passes `root.Name()`, which works because
`fsroot.Dir` exposes the full path (`Name()`) where stdlib `os.Root` hides it. `pkg/sops` takes it as a `string` for
now; now that `pkg/fsroot` is a standalone package (see [`root-extraction.md`](root-extraction.md)) the `Encrypter` can
upgrade to a typed `fsroot.Dir` — a deferred one-line change.
