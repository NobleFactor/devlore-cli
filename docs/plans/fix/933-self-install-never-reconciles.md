...
- [ ] `docs/architecture/` records the model beside writ's, so the two read as one: a record of what
      is owned, and an operation that replaces it
- [ ] `docs/cli/` pages for `self install` regenerate with the behaviour
