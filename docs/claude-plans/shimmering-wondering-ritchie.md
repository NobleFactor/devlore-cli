...
### Done: Part 23b — Rename `shell.Provider.Shell` → `Exec`

### Done: Part 23d — Wire UI output functions in `builder.go`

`prepareScriptEnv()` wires `note`, `warn`, `error`, `success`, `fail` via
`ui.Provider` with `makeUIBuiltin` / `makeUIErrorBuiltin` helpers.

### Done: Predicate methods on providers (replaces old Part 23c)
...
```
cd noblefactor-ops && go build -o star ./cmd/star
```
...
This file must be regenerated or manually restored to match the current set of
plannable providers.

### Step 3: Add `go.return_strings(scope)` to noblefactor-ops

New AST primitive for Part 23e. Extracts `[]string{...}` literal elements from
...
- `compensation_test.go` was deleted (referenced deleted types) — needs review
