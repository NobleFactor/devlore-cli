...
`Resolve(root)` populates provider-specific metadata via scoped I/O
(e.g., `os.Stat` for files, compilation for callables). The `Root`
parameter provides the scoped filesystem boundary for confined
execution. If resolution changes identity-bearing fields (e.g., path
canonicalization), the cached URI is updated.
