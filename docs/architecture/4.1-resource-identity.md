...
See also:
- [4-resource-management.md](4-resource-management.md) — catalog, shadows, tombstones, resolution lifecycle
- [4.2-mem-resource.md](4.2-mem-resource.md) — `mem:` scheme and callables
...
Each concrete type constructs its URI at creation time and caches the
result — identity does not change after construction.

`Resolve()` locates the resource by URI and verifies reachability,
enriching metadata (stat, version) via scoped I/O — the filesystem
boundary comes from the bound root. Resolution never changes identity
(amended 2026-08-20 — activation is a state-and-binding event): a
scheme whose identity is a rel re-binds its location rel-first through
the `op.RootBinder` seam (`BindRoot(root fsroot.Dir)`), and the cached
URI is untouched.
...
- **Hierarchical**: starts with `//` after the scheme. Go's `url.Parse`
  ...
  for the `file` scheme's *rendered* form against a bound root
  (`file:///etc/foo`); `file` catalog identity is opaque (§3).
...
**Function type** — the named Go function type the callable adapts to
(e.g., `file.Reducer`, `Predicate`). Discovered by the codegen from
provider method signatures. For anonymous function types (bare `func(...)`
parameters without a named type), the fallback is `<action>.<param>` —
the action method name and the parameter name that receives the callable.
Named types are the primary path.
...
|---|---|---|---|
| `file` | Opaque (catalog identity) / Hierarchical (rendered) | catalog: `file:etc/foo` — the slash-canonical **root-relative** path; rendered against a bound root: `file:///etc/foo` | **Amended 2026-08-20** ([resource-construction](../plans/resource-construction.md)): catalog identity is the rel — the fsroot is a run parameter, so no volume, home layout, or separator variance enters identity; a plan-authored leading slash anchors at the fsroot (git model) and normalizes away. The [RFC 8089](https://datatracker.ietf.org/doc/html/rfc8089) absolute form survives only as the *display/rendered* form once a run binds the root. |
| `pkg` | Opaque | `pkg:brew/jq`, `pkg:deb/curl`, `pkg:winget/Microsoft/VSCode` | [purl](https://github.com/package-url/purl-spec)-compliant, **versionless** — the catalog URI omits `@version` (version is state, see [§3.4](3.4-platform-package-managers.md)). `Purl()` may append the requested `@version` for display, so `URI()`/`Purl()` converge only on the versionless coordinates. |
...
```go
// file: catalog identity is the slash-canonical root-relative path (amended 2026-08-20 — the fsroot is a
// run parameter, so identity carries no volume, home layout, or separator variance; the RFC 8089 absolute
// form is the rendered form against a bound root, never the identity).
func (r *Resource) buildURI() string {
    return "file:" + r.SourcePath.Rel() // rel is the half that serializes
}

// pkg: purl-compliant opaque URI — VERSIONLESS catalog identity.
// Version is state, not identity (§3.4): `git` and `git@2.39.0` are the same
// catalog entry. r.Version is the *requested* version the reconciler converges
// to; it rides in Purl() for display, never in the catalog URI. r.Type is the
// manager, normalized with the platform default at construction.
func (r *Resource) buildURI() string {
    return "pkg:" + r.Type + "/" + r.Name
}
...
The URI never changes after construction (amended 2026-08-20):
activation binds the root without touching identity, and `Resolve()`
enriches metadata only.
...
For opaque URIs — `pkg:`, `svc:`, `appnet:`, `mem:`, and `file:`
catalog identities (amended 2026-08-20: `file:<rel>` is opaque) —
`Opaque()` returns the data after the scheme colon; `Host()` and
`Path()` return empty. For hierarchical URIs (e.g. the `file:///…`
rendered form), `Opaque()` returns empty; `Host()` and `Path()` return
the authority and path components.
...
(pinned) — a metadata refinement, not an identity change: the catalog
strips the fragment when keying.
...
|---|---|---|---|
| [4-resource-management.md](4-resource-management.md) | §3.3 Resource Types by URI Scheme | **Extended** | Scheme table updated; URI construction model changed |
| [4-resource-management.md](4-resource-management.md) | §2 Architectural Summary | **Extended** | Resource interface simplified |
