...
func evaluateMigrateCorrectness(analysis *migrate.MigrationAnalysis,
    graph *op.Graph, expected MigrateExpected) CorrectnessMetrics {
    ...
    // Rename operations (set comparison over the sealed graph's move nodes)
    expectedRenames := toSet(expected.RequiredRenames)
    actualRenames := toSet(extractRenames(graph))
