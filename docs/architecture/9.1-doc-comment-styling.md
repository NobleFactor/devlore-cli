...
> **Arrived from noblefactor-ops 2026-09-24** (#937). star was a product of that repository until
> March 2026; its code came here and its design did not. This is the only written design of the
> `goast` doc-comment styler, which ships in `star lint go-style`.
>
> **As-is against this code:** the production model (`production.go`), fuzzy slot filling
> (`fuzzy.go`, `slotassign.go`), and the schema fields — `production`, `consumes`, `condition`,
> `prefix`, `split`, `slots` — that `com.noblefactor.star.LintGoStyle/extension.yaml` carries.
>
> **Not as-is: the "Porting to devlore-cli" section below records an unfinished migration. It is not
> a set of instructions to follow.** Five items it lists as porting as-is are absent —
> `resizeProduction`, `regionProduction`, `BoxStyle`, the production registry (`RegisterProduction` /
> `ProductionFactory`) and `MultiCommentProduction`. The ~90 lines of reflection it says the port
> deletes are still in `astrewrite.go:20-64`. `SchemaElement.Style` and `FormStyles` do not exist.
> Tracked as [#938](https://github.com/NobleFactor/devlore-cli/issues/938), which also records that
> `NewProduction`'s `default` branch silently returns an item production for a production it does not
> recognize. Read that section as history.
