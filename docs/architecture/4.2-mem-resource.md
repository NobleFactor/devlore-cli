...
**`mem.Resource` is a sealed interface**: an exported interface embedding `op.Resource`, sealed by an unexported method
it declares itself, over an unexported struct in the provider's package. Nothing outside the `mem` package can build one, which
...
See [4.1-resource-identity.md](4.1-resource-identity.md) for
the full URI design.
...
thread — the execution context for `starlark.Call`. Thread custody lives with
`starlarkbridge.Invoker`, the per-session invocation service registered on the
`RuntimeEnvironment` via the service locator (never a process singleton); an
action that invokes a callable reaches the invoker through its activation's
runtime environment. (The pre-`op` sketch — a `Thread` field on the deleted
`op.Context` — is gone.)
...
|---|---|---|---|
| [4-resource-management.md](4-resource-management.md) | §3.3 Resource Types | **Extended** | `mem:` scheme now implemented; row updated from "Planned" to "Implemented" |
| [2.3-orchestration-primitives.md](2.3-orchestration-primitives.md) | Runtime Predicates | **Superseded** | `RuntimePredicate` type replaced by `mem.Callable` + `PredicateAdapter` |
| [2.3-orchestration-primitives.md](2.3-orchestration-primitives.md) | Serialization §"What Doesn't Serialize" | **Superseded** | Callables now serialize via source text + compiled bytecode |
| [2.1-typed-slots.md](2.1-typed-slots.md) | Slot Types | **Extended** | `mem.Callable` flows through existing immediate slot path as a Resource |

## 7. Related Documents

- [4-resource-management.md](4-resource-management.md) — Resource lifecycle, catalog, URI scheme table
- [2.3-orchestration-primitives.md](2.3-orchestration-primitives.md) — RuntimePredicate (superseded by callable)
- [2.1-typed-slots.md](2.1-typed-slots.md) — Slot model and resolution
- [2.2-phase-execution.md](2.2-phase-execution.md) — Saga pattern, recovery stack
