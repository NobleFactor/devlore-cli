...
> **Arrived from noblefactor-ops 2026-09-24** (#937). The migration it proposes **succeeded** — the
> Rust WASM gitignore extension is gone, and `star-wasm-receivers.md`, which designed the runtime it
> needed, was deleted in noblefactor-ops#230 because no repository holds that subsystem any more.
>
> **As-is against this code:** the ignore stack, verbatim. `pkg/gitignore/tracker.go` layers the
> global `core.excludesfile`, then `.git/info/exclude`, then the root `.gitignore`, pushes a source
> per directory and lets the deepest match win, pairing each pattern set with the path it came from.
> The design decisions hold and are recorded nowhere else: go-git over sabhiram, no shelling out to
> `git`, and no parallel walk yet.
>
> **Not as-is: the entire Starlark and Go API section.** The package is `pkg/gitignore`, not
> `internal/ignore`. There is no `walker.go` or `global.go`. `WalkTree` is a **file-provider method**
> returning `(product any, stack *op.RecoveryStack, err error)` with a `CompensateWalkTree`
> companion — a receipt model this document predates. Its Starlark signature is
> `file.walk_tree(root=…, fn=<Reducer>, include_gitignored=…)`, a **fold**, not a callback over
> `{path, name, is_dir}` returning `"skip"` / `"stop"`; the flag name and sense are both different.
> `file.tracker()` does not exist. `ignore.WalkTree(ignore.WalkOptions{…})` does not exist.
>
> **Placement:** this sits under §9 because that is where the star record came from. Its subject is
> `pkg/gitignore` and the file provider, which is framework rather than star, so
> `3.5.4-file-provider.md` is arguably its home. Moving it is a rename.
...
- Preserve the Rust WASM source as a reference example for building WASM receivers
  (linked from the Writing Extensions guide (noblefactor-ops's, disposed of by noblefactor-ops#228))
...
Writing Extensions guide (noblefactor-ops's, disposed of by noblefactor-ops#228), demonstrating the
