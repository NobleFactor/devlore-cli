...
- [x] **PowerShell modeled on runspaces, not processes** — a PowerShell-side host owning a `RunspacePool`, with §5's
...
- [ ] Reconcile with [`2.4`](2.4-hermeticity-guarantees.md) — a reused host is an unmodeled channel between units.
...
- `NeedsSudo()` is modeled per backend but has nowhere to go except a boolean parameter.
