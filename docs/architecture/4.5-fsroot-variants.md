# fsroot — One Sandbox, Two Types

> **Status:** design (draft 2026-08-18, rewritten 2026-08-19). Records what `pkg/fsroot` is for and what it adds
> to `os.Root`. An earlier draft described three sandbox variants; that design is replaced here by a single
> sandbox behavior, for the reasons in §5. No implementation. Companion:
> [`4.5-fsroot-variants.status.md`](4.5-fsroot-variants.status.md).
>
> **Relationship to the neighbors.** [`4.4-root-path-triad.md`](4.4-root-path-triad.md) owns the cooperative
...
**There is one sandbox behavior, and it is the kernel's.** fsroot exists not to offer a choice of
sandbox but to supply the five things `os.Root` lacks — most importantly, permission enforcement that means
something on Windows.

An earlier draft defined three variants differing in *what is unsandboxed*. Working through their consumers
found none: every production caller of the unsandboxed variants anchors at a tree it owns and writes only inside
...
reason to abandon sandbox (§3).

## 1. The two types

| Opener | Type | Lifetime |
| --- | --- | --- |
| `OpenExisting(dir)` | `sandboxedDir` | the caller's |
| `OpenScratch(pattern)` | `sandboxedScratchDir` | removes its tree on `Close` |

Both sandbox reads and writes to their root, by the kernel, through `*os.Root`.
...
No `Mode`, and no mode-dispatched constructor. With one sandbox behavior there is nothing to select, and a
...
translates the one distinction Windows can honor — private versus not — into a **protected** DACL granting the
...
precisely the defect class [#547](https://github.com/NobleFactor/devlore-cli/issues/547) catalogs.
...
**5. Scratch lifetime.** A sandboxed tree that removes itself, so process scratch flows through the same seam as
every other mutation and inherits its platform behavior.
...
**So a caller whose target does not exist yet** opens a sandboxed root at an existing **ancestor**, creates the
chain through it, then opens the target root. `/` always exists, so an ancestor always does. Both steps stay
sandboxed, the mode is applied on the way, and the creation is visible at the call site rather than implied by
an open.
...
**Residual, accepted:** the link exists on disk, and anything outside devlore's sandboxed I/O that follows it
...
sandboxed by explicit choice: `pkg/op/provider/plan/provider.go:488` and `cmd/writ/writ/deploy/plan.go:387` both
pass a sandboxed mode today. And `file.Provider.Link`'s `verbatim` flag **survives** — writing an archive's
target as authored is simply correct once containment is the kernel's business.

## 5. Why the unsandboxed variants go

The evidence, gathered 2026-08-19:

**Nothing reads outside its own tree.** Every production `OpenWritableUnsandboxed` anchors at
...
unsandboxed because the prefix need not exist yet — a first install creates it — and a sandboxed root requires
...
absolute path the sandboxed Root is anchored at". System scope targets `/`, and a sandboxed root at `/` bounds
nothing on Unix, so 2.4's "System is unsandboxed" is a degenerate case of sandbox rather than a second
variant.

**And "read from elsewhere, write here" wants two roots, not one half-sandboxed one.** Two sandboxed roots state
the actual intent — read tree A, write tree B. A write-sandboxed root grants **unbounded** reads to express a
**bounded** need, which is strictly more authority than any caller has asked for.

### 5.1 The work

1. **Verify the premise.** Trace every `OpenWritableUnsandboxed` call site and confirm none writes outside its
   anchor. Mechanical, and the only thing that can invalidate this design. **Do this first.**
2. **Migrate the call sites.** Tests against `t.TempDir()` become `OpenExisting` directly. The bootstrapping
   cases — prefix, ConfigHome, StateHome, CacheHome, installPath — create through an ancestor root, then open.
3. **Delete** `unsandboxedRootReader`, `unsandboxedRootReaderWriter`, `rootBase`, `errReadOnly`, `Mode` and its
   constants, `Open`, `OpenUnsandboxed`, `OpenWritableUnsandboxed`.
4. **Rename** `sandboxedRoot` → `sandboxedDir`, `scratchRoot` → `sandboxedScratchDir`.
5. **`WithRoot(path, mode)` → `WithRoot(path)`**, and `WithRootMode` with it.

Five types become two. The `Dir` interface stops being a substitution point for sandbox policy and survives
only as the seam every provider already programs against.

## 6. Open questions

1. **Does any current unsandboxed caller write outside its anchor?** §5.1 step 1 — the only open item, and a
