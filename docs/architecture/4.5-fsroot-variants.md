...
> sandbox behaviour, for the reasons in §5. No implementation. Companion:
...
**There is one sandbox behaviour, and it is the kernel's.** fsroot exists not to offer a choice of
...
No `Mode`, and no mode-dispatched constructor. With one sandbox behaviour there is nothing to select, and a
