...
The file provider (`pkg/op/provider/file`, announced `op.RoleModule|op.RoleAction`) is the
...
`file.mkdir` is at the seam too (#822, 2026-09-20): a non-directory occupant of its path -- a file, or a symlink live
or dangling -- is an occupied target, and under `replace` the occupant is archived before the directory is created,
with the receipt restoring it on compensation; a directory at the path is the idempotent case, never a conflict.
