...
and configure from — so #2's path-keyed child-type schema (typing `brokers.ssh`, `brokers.ssh.step-ca`, …) and the
...
```go
// elevation.Broker — the provider's broker interface. Any type that satisfies it is a broker; the type
// may live anywhere, and a router is just a broker that delegates to sub-brokers instead of doing the work.
type Broker interface {
    Mint(ctx context.Context, offer Offer) (*Lease, error)
    Revoke(ctx context.Context, lease *Lease) error
}

// The elevation provider builds its router from config; the router allocates ssh/sudo by its own rule.
func NewProvider(runtimeEnvironment *op.RuntimeEnvironment) (*Provider, error) {
    configuration, ok := devconfig.SectionOf[*ProviderConfig](runtimeEnvironment.Application.Config)
    if !ok {
        return nil, fmt.Errorf("elevation: missing config section")
    }
    router, err := newElevationRouter(configuration.Brokers) // a Broker; allocates+configures its sub-brokers
    if err != nil {
        return nil, err
    }
    return &Provider{ProviderBase: op.NewProviderBase(runtimeEnvironment), broker: router}, nil
}
```
...
Selecting a *specific* broker for an individual request is the router's **dispatch** (POSA's Lookup), distinct from
allocation: the router builds its sub-broker set from config, then chooses among them per request.
