...
- **Register — the provider registers its configured brokers.** *Register* means **allocate and prepare for use.** At
  **provider construction time** the provider hands its configured broker sub-tree to the **broker trait**, which looks
  each one up among the announced set and allocates it ready to serve. Announcement says what *can* exist; configuration
  says what *does*. The allocation loop lives in the trait, not in each provider — a brokered provider writes one call.

A broker is therefore a **declaration that a type serves as a broker**: `op.AnnounceBroker` registers it under the
provider's dotted namespace (`elevator.brokers`), and the type's only obligation is to fulfill the provider's broker
contract — no shared base, no marker interface. One declaration feeds **two registries** keyed by the same name: the
`devconfig` config-section schema (so the loader types the broker's config) and the `op` runtime factory (so the trait
can allocate it).
...
[Configuration — the elevator case study](configuration.md#case-study-the-elevator-section)). The provider hands that
sub-tree to the trait at construction; the trait allocates exactly the brokers it names.

```go
// The broker trait, selected only by providers that have brokers — it owns the allocation loop.
type Provider struct {
    op.ProviderBase
    op.BrokerRegistry          // opt-in: brokered providers embed it; others do not
}

// A brokered provider writes one call: hand the resolved broker sub-tree to the trait, which
// looks up each announced factory and allocates the configured brokers (register = allocate + prepare).
func NewProvider(env *op.RuntimeEnvironment) (*Provider, error) {
    ...
    if err := p.RegisterConfigured("elevator.brokers", cfg.Brokers); err != nil {
        return nil, err
    }
    return p, nil
}
