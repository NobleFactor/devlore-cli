...
**Interface-typed members project through the interface, not its implementers.** When a field or return is an
interface (`Decls []Decl`, or a method returning `Decl`), the starlark surface of that slot is **exactly the
interface's own method set** (`DeclName`, `DeclKind`, `DeclComment`, `DeclStyle`) — nothing concrete-specific is
reachable, because the static type is the interface. So the generator **reflects on the interface declaration**
(Go interfaces spell out their method set explicitly) and registers the *interface* as a type, tagging
`+devlore:property` on its methods where warranted (e.g. `DeclKind`). At runtime the bridge resolves an
interface-typed value's projection from the **interface's** registration — its `MethodMetadata` (parameter names,
the property `Modifiers`) — while the method itself dispatches on the concrete value via reflection. The concrete
implementers (`GenDeclNode` / `FuncDecl` / `CommentDecl`) need no registration *for that slot*; they ride the
interface contract. Enumerating implementers would be solving a discovery problem that does not exist: the
contract lives on the interface, and reflection already handles dispatch to whatever concrete value satisfies it.

> This requires the bridge, for an interface-kind value, to resolve the receiver type from the interface type
> *before* unwrapping to the concrete value (`toStarlarkReflect` currently `elem`-unwraps first and resolves the
> concrete type), and `goast` to expose an interface's declared method set (today it only counts interfaces). Both
> are lighter than an implementer graph query — register one declaration, read one method set.
