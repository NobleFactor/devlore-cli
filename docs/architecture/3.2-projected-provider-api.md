...
`op.ResolveReceiverType`. The concrete is the single source of both **dispatch** (its methods carry receiver-bound
...
`Decl`?" query. At runtime the unwrap-to-concrete + `ResolveReceiverType` path finds the registered concrete with
