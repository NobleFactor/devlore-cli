...
```text
elevator router   (elevation.Broker)        ← provider builds it from base.providers.elevator.brokers
...
// The elevator provider builds its router from config; the router allocates ssh/sudo by its own rule.
func NewProvider(runtimeEnvironment *op.RuntimeEnvironment) (*Provider, error) {
    configuration, ok := devconfig.SectionOf[*Config](runtimeEnvironment.Application.Config)
    if !ok {
        return nil, fmt.Errorf("elevator: missing config section")
    }
