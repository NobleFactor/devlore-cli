...
- [Action Namespaces](3-operation-namespaces.md) ([status](3-operation-namespaces.status.md)) — How to add a namespace: the hand-written provider contract, `make generate` (announcement + action-name constants + inventory), tests, and the catalog/design-doc step
  ...
  - [Method Claims — deterministic, idempotent, sandboxed](3.6-method-classification.md) ([status](3.6-method-classification.status.md)) — a provider method states the guarantees it makes; codegen refuses a claim its signature contradicts and `pkg/op/claimcheck` holds the rest to the call graph. Mutation is the one thing derived rather than claimed, since a receipt return cannot drift. Surface eligibility is a separate declaration, and hermeticity a per-method filter applied per runtime
