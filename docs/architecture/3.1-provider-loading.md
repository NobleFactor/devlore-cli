...
> **Status note (2026-07-22, step-51 exit gate):** the **module-loading half is current** — `With()`
> pre-injection, `load("@devlore//…")`, aliasing, and the loader cache exist in `pkg/op/starlarkbridge`. The
> **lifetime half is design that did not land as written**: the `LifetimeStateless` / `LifetimePhase` /
> `LifetimeSession` taxonomy and the descriptor-based Registration Examples (`op.Announce(&…{})` / `InitAll`) have
> no tree presence — the landed lifetime model is environment-scoped construction (a provider is built against its
> `*op.RuntimeEnvironment` and cached on it; [4-resource-management.md](4-resource-management.md) §8, provider lifecycle), and
> registration is announce-with-metadata ([3-operation-namespaces.md](3-operation-namespaces.md),
> [4.3](4.3-resource-registration.md)). **The full rewrite is recorded step-51 residual debt** (gate finding 1) —
> read the lifetime sections below as an unrealized proposal until then.
>
> **Ruled 2026-08-22 (directive inventory, [resource-construction](../plans/resource-construction.md) phase 3):
> the `+devlore:lifetime` directive is REMOVED, with the dormant `Lifetime` machinery**
> (`pkg/op/provider/lifetime.go` and the generator's parsing) — the landed environment-scoped model is the
> lifetime model. The lifetime sections below are now a rejected proposal, kept only until the step-51 rewrite
> disposes of them.
...
3.2-projected-provider-api.md#registration)):
...
`Lifetime` follows this declaration model:
...
Registration](3.2-projected-provider-api.md#registration) for
...
3.2-projected-provider-api.md#registration). The `init()`
