...
#### Star's as-is surface, salvaged 2026-09-24

Two tables from noblefactor-ops' `star-configuration.md` (#937). The model and rationale above were
already here; these two were not, and that document is deleted by noblefactor-ops#228. **Type names
updated to today's**: `ConfigSpec` is `config.Spec`, `ConfigElement` is `config.Element`,
`ConfigAccessor` is `config.Accessor`, `ConfigValue` is `config.Value`, and the package is
`cmd/star/config`, not `internal/config`.

The field-type strings a `config.Spec` accepts:
...
Nested types use the `Nested` field on the spec.

And what `config.sync` generates, reading through `config.Accessor`:
...
Each generated file carries the header `# GENERATED BY star lint sync - DO NOT EDIT`
(`cmd/star/config/sync.go`).
