...
reaction). **Condition** — the run's health, latching, orthogonal: `healthy` → `degraded` → `failed_execution` →
...
as latched. **Reason** — the prose driver of the latest move, carried on the latched triplet for informative logs.
