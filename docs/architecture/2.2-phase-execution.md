...
**State** — the run's health, latching: `healthy` → `degraded` → `failed_execution` → `failed_compensation`. On
each aberrant flip the run continues, pauses, or stops by an act of configuration (`failed_compensation`: always
...
A run ends in one of **four derived terminals** — `stopped ×` {`healthy`, `degraded`, `failed_execution`,
`failed_compensation`}:
...
- **FailedExecution** — the run stops while `failed_execution` (renamed from "failed" 2026-07-05, for symmetry
  with `failed_compensation`): a saga boundary exhausted its retry policy, or `flow.Failed` executed, and the
  unwind was clean; the system is back at its pre-run state.
- **FailedCompensation** — an unhandled failure whose **unwind itself failed** (step 4). The system is half-changed, so this
  is *categorically* worse than `FailedExecution`: it fails loudly, persists the execution journal, and emits restart
