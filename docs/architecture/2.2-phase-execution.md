...
Run state is a pair of orthogonal dimensions (settled 2026-07-05, refined 2026-07-06; realization: phase-8 step
41). **Phase** — where the run is: `preparing` (construction + pre-flight: variable binding, environment build,
catalog clone) → `running` (from the first unit dispatch) → `pausing`/`paused` (resumable), and **two terminal
phases**: **`completed`** — the natural end (the final unit executes, or `flow.Complete` executes; the result is
Complete's input) — and `stopping` → **`stopped`** — the commanded or policy-driven end (stop command,
cancellation, or a `TransitionPolicy` Stop reaction). **State** — the run's health, latching, orthogonal:
`healthy` → `degraded` → `failed_execution` → `failed_compensation`. **Completing is not a State transition** —
the run is done and State remains exactly as latched. On each aberrant flip the run continues, pauses, or stops by
an act of configuration (the `TransitionPolicy`; floors: degraded → continue, failed_execution → stop,
failed_compensation → stop with pause legal and continue illegal). On stop the run returns the final action's
result and error plus the terminal state (phase + state).
