...
Run state is a two-axis state machine (settled 2026-07-05; realization: phase-8 step 41). The **execution axis**:
`preparing` (construction + pre-flight: variable binding, environment build, catalog clone) → `running` (from the
first unit dispatch) → optionally the **aberrant running states** — `running, degraded` (default **continue**; stop
by an act of configuration) and `running, failed` (default **stop**; continue by an act of configuration) — until a
terminal driver fires. The **control axis** superimposes on any running state: `paused` (resumable; preserves
position and aberrance) and `stopped` (command-driven terminal unwind; a failed stop-unwind lands
`CompensationFailed`). On stop the run returns the final action's result and error plus the terminal state.

A run ends in one of **four terminals** — `Completed`, `Degraded`, `Failed`, `CompensationFailed`:
...
- **Failed** — the run stops while failed: a saga boundary exhausted its retry policy, or `flow.Failed` executed,
  and the unwind was clean; the system is back at its pre-run state.
