...
## Run States and Terminals — the State Machine

Run state is a pair of orthogonal dimensions (settled 2026-07-05; realization: phase-8 step 41). **Phase** — where
the run is: `preparing` (construction + pre-flight: variable binding, environment build, catalog clone) → `running`
(from the first unit dispatch) → `pausing`/`paused` (resumable) and `stopping`/`stopped` (the sole terminal phase).
**State** — the run's health, latching: `healthy` → `degraded` → `execution_failed` → `compensation_failed`. On
each aberrant flip the run continues, pauses, or stops by an act of configuration (`compensation_failed`: always
stop). Terminals are derived, not enumerated: `stopped × <state>` — `stopped, healthy` is the completed run. On
stop the run returns the final action's result and error plus the terminal state (phase + state).

A run ends in one of **four derived terminals** — `stopped ×` {`healthy`, `degraded`, `execution_failed`,
`compensation_failed`}:

- **Completed** — the last unit executes, or `flow.Complete` executes (the result is Complete's input — anything).
- **Degraded** — the run stops while degraded. `flow.Degraded` (a gate on its input) drives the aberrant state; a
  unit's error action is a `*Subgraph` that **MUST run** on failure, and a consumer opts into degrade-and-continue
  by placing a `flow.Degraded` node in it (see [§2.3](2.3-orchestration-primitives.md)) — successes are kept,
  failures recorded, no unwind.
- **ExecutionFailed** — the run stops while `execution_failed` (renamed from "failed" 2026-07-05, for symmetry
  with `compensation_failed`): a saga boundary exhausted its retry policy, or `flow.Failed` executed, and the
  unwind was clean; the system is back at its pre-run state.
- **CompensationFailed** — an unhandled failure whose **unwind itself failed** (step 4). The system is half-changed, so this
  is *categorically* worse than `ExecutionFailed`: it fails loudly, persists the execution journal, and emits restart
...
The full protocol — error-action dispatch, best-effort fan-out, the `CompensationFailed` journal and restart — is in
