...
reaction). **Condition** — the run's health, orthogonal — it only worsens, never improves within a run: `healthy` → `degraded` → `failed_execution` →
`failed_compensation` (Go identifiers read subject-verb, e.g. `ConditionExecutionFailed`, while the serialized names
keep these snake forms). **Completing is not a Condition transition** — the run is done and Condition remains exactly
as as it stood. **Reason** — the prose driver of the latest move, carried on the status for informative logs.
On each aberrant flip the run continues, pauses, or stops by an act of configuration (the `TransitionPolicy`;
floors: degraded → continue, failed_execution → stop, failed_compensation → stop with pause legal and continue
...
- **`completed × failed_execution`** — ran to the end despite a failure: exactly what a Continue reaction on
  `failed_execution` produces.
- **`stopped × healthy`** — canceled/stopped cleanly before finishing (distinct from completion).
- **`stopped × failed_execution`** — the default stop-on-failure end: a saga boundary exhausted its retry policy,
  or `flow.Failed` executed, and the unwind was clean; the system is back at its pre-run state.
- **`stopped × failed_compensation`** — an unhandled failure whose **unwind itself failed** (step 4). The system
  ...
  (observe, don't assume). `failed_compensation` pairs only with `stopped` — continue is illegal there.
