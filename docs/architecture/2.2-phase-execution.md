...
The recovery pointer stack is a **tree of compensators**: a leaf is a **receipt** (one action's undo evidence), a
...
- Serialize the tree as **data** with a `kind` discriminator; the compensating action stays a **name** resolved at undo
  time (closures do not serialize).
...
free-text message (settled 2026-07-05, refined through the 2026-07-09 reconciliation; realized in phase-8 step 41, landed 2026-07-11). **Phase** — where
...
|--------|------|--------|-----------|
| **Forward** | Execute the operation | The method itself: `Copy`, `Install` | `func(params...) (result T, compensator U, error)` |
| **Backward** | Compensate the operation | `Compensate` prefix: `CompensateCopy` | `func(compensator U) error` |
...
| **compensator** | Second | Backward | Compensation receipt — the S in (A, C, S). Opaque to the executor. Provider-specific type (e.g., `Tombstone`) |
...
Non-compensable methods (no `Compensate` pair) omit the compensator return.

### Backward

Backward receives the compensator saved by Forward and undoes the operation:

```
Forward(params...)  → (result, compensator, error)
                         ↓        ↓
                       graph    saved on node
                                   ↓
Backward(compensator) → error
```

- Backward has no result. Compensation is terminal — nothing downstream
  consumes its output.
- Backward has no compensator. There is no "compensate the compensation" — the
  saga pattern does not nest.
- If Backward needs Forward's result, Forward saves it in the compensator.
  Compensator is the single channel from Forward to Backward.
- If Backward needs Forward's params (e.g., which packages to remove), Forward
  saves them in the compensator.
...
`reflect.Method.Func.Call()`. The reflected `Undo` passes the compensator
...
- **Gate 2**: If a `Compensate<Method>` pair exists, its signature must
  be `func(compensator T) error` (exactly 1 parameter + error return)
- **Gate 3**: Orphan `Compensate` methods (forward in params but not consumed)
  trigger a panic — pairing bugs are caught at startup

### State Capture

The reflected action captures compensator from the Provider's Forward return
and returns it from `Do`. The executor stores compensator in a
`RecoveryEntry` on the recovery stack.

Compensator is **opaque to the executor**. The executor stores it and passes it
...
Forward execution:
  ...
  4. Forward returns (result, compensator, error)
  5. Do returns (Result, Compensator, error) to executor
...
Compensation (on failure):
  ...
  9. for each completed node (reverse order within phase):
     ...
     → reflected action calls provider.Compensate<Method>(compensator) via reflection
     → Provider reads compensator, decides what to undo
```
...
### Resolved: Compensator Serialization

Compensator is provider-specific (e.g., `file.Tombstone`, `pkg.Tombstone`).
The Provider owns its compensator type and decides what it needs. Implementers
are responsible for ensuring their compensator round-trips through JSON and
...
| `pkg/op/action.go` | Action, FallibleAction, CompensableAction, Result, Compensator |
