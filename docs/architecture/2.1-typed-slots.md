...
Capability functions like `decryptor` are not filled from Starlark arguments — they come from Context.Data at
runtime. A Starlark *function* passed as a method argument (e.g. `walk_tree(fn=…)`) is different: it converts to a
content-addressable `function.Resource` at plan time and to the Go callable at runtime — see
[Plan-Time Conversion](#plan-time-conversion) below.
...
`AddressingContent` resource converts along its *content* (`function.Resource ⇄ bytecode / func`). The planner enforces
this with a two-case switch on `Resource.Addressing()` — any other addressing is `assert.Unreachable`.

**The platform comes for free.** Plan-time resource construction needs a `Platform` (e.g. `pkg.Resource` resolves its
purl type from the platform — the manager→purl-type matrix lives in `pkg/platform`, covered by `purl_type_test.go`).
`NewRuntimeEnvironment` defaults `Platform` to `platform.Detect()` when none is set, so no caller touches
`WithPlatform`. Execution always runs on the detected host; `WithPlatform` is only an explicit override for
cross-platform planning (build a Linux graph from a Darwin host).
...
`*starlark.Function` argument becomes a content-addressable `function.Resource` via `function.NewResource(env, …)`, and
Resource construction consults `env.ReceiverRegistry`. Keeping the environment on the converter makes it available
wherever a value is converted — at any nesting depth, since a function nested in a list or dict is reached by the same
recursive `toNaturalGo`.
...
arguments of a method call into the Go types its provider method expects, and some of those are resources that need the
environment — so every receiver needs the converter. The plan provider converts identically, from the same session
environment.
...
`file.walk_tree(fn=…)` hands `file.Provider.WalkTree` a Starlark reducer it calls per entry. The function is captured at
plan time as a content-addressable `function.Resource`; at execution `op.Convert` turns it into the `file.Reducer` the
method expects via `reflect.MakeFunc` — a manufactured Go func whose body converts its arguments *to* Starlark, calls
the function, and converts the result *back*. That conversion is the mirror of *The Converter* and uses the same single
implementation — notably wrapping a `*file.Resource` argument as a `goReceiver` so the reducer can receive it.

`op` and the resource packages stay Starlark-agnostic. The boundary-untangle removes the `starlarkbridge → function`
edge, so `function` can import `starlarkbridge`, and the calling machinery lives in `starlarkbridge` itself as
**`starlarkbridge.Invoker`** — defined and implemented there, consumed by `function`:
...
type Invoker interface {
    ...
    ToGoValue(value starlark.Value) (any, error)         // Starlark → Go
}
```

It rides on the `RuntimeEnvironment` as a generic **service** (`services map[reflect.Type]any`, reached via
`ServiceFor` / `ServiceFor[T]`), so `op` carries it without naming it. The Starlark runtime registers the
implementation on the planning env and the graph executor on the execution env (both under
`reflect.TypeFor[starlarkbridge.Invoker]()`); the `function.Resource` pulls it at conversion time. There is exactly one
Go↔Starlark converter — the bridge — and providers never roll their own. Each call runs on a fresh Starlark thread:
threads are not safe for concurrent reuse, so the bridge mints one per call rather than sharing a session-wide thread.
