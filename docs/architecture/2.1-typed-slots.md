...
`op` and the resource packages stay Starlark-agnostic, and `starlarkbridge` already depends on `function`, so the
calling machinery is a small neutral contract, **`starlark.Bridge`**, that `function` consumes and `starlarkbridge`
implements:
...
type Bridge interface {
    ...
    ToGoValue(value gostarlark.Value) (any, error)         // Starlark → Go
}
```
...
`reflect.TypeFor[starlark.Bridge]()`); the `function.Resource` pulls it at conversion time. There is exactly one
