...
method expects via `reflect.MakeFunc` — a manufactured Go func whose body marshals its arguments *to* Starlark, calls
the function, and marshals the result *back*. That marshaling is the mirror of *The Converter* and uses the same single
...
`ServiceByType` / `ServiceFor[T]`), so `op` carries it without naming it. The Starlark runtime registers the
...
Go↔Starlark marshaler — the bridge — and providers never roll their own.
