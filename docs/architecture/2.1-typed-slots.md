...
`function.Resource` builds its own Invoker — `starlarkbridge.NewInvoker()` is env-free (its converter never reads the
runtime environment on the conversion path), so each resource holds an `invoker` field set at construction and
`ConvertTo` calls through it. No service locator, no registration on the `RuntimeEnvironment`. There is exactly one
...
- **reflectedCompensableAction** — Calls the method, classifies the return as
  `(result, compensator, error)`. `Undo` calls `Compensate<GoName>` via reflection,
  passing the compensator directly.
