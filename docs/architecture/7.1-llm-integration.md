...
> **Status:** re-grounded 2026-07-22 (phase-8 step 51, slice 6) on the rewritten `writ migrate` (phase-8 steps
...
`lore config model` manages the persisted selection; commands requiring an AI provider fail with a pointer to it
...
**Response Parsing — the LLM proposes; Go builds the graph:**

The response is a flat proposal — analysis fields plus an operation list (`{action, source, target, project,
reason}`) and ordering edges (`{from, to}`) — parsed by `parseRegistryLLMResponse` into
`LLMResult{Analysis *MigrationAnalysis, Graph *op.Graph}` (`cmd/writ/writ/migrate/plan.go`). The **sealed graph is
never parsed from LLM output**: `planBuilder` (`plan_builder.go`) plans each proposed operation as a file-provider
invocation (`Mkdir` / `Copy` / `Rename` / `Remove`), records the proposed edges as `DependsOn` constraints,
topologically sorts, and assembles via `plan.Provider.AssembleDefinition` inside an `op.Plan` closure
([3.5.3](3.5.3-plan-provider.md)) — so every LLM proposal passes the same validation, orphan scan, and seal as any
hand-written plan, and a malformed proposal fails plan construction rather than reaching execution.
...
func TestMigrationWithOllama(t *testing.T) {
    ...
    assert.Equal(t, "tuckr", string(result.Analysis.System))
    assert.Greater(t, len(result.Graph.Nodes()), 0) // the Go-built sealed graph
}
```
