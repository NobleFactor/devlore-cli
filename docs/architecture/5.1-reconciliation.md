...
```go
Do(ctx *Context, slots map[string]any) (Result, Complement, ReconciliationState, error)
```
...
| Complement | Recovery Stack | Mechanical reversal if the graph fails |
...
#### Reconcile, `CompensationFailed`, and Restart

The reconcile gate is also what makes restart after a *failed* unwind safe. When a compensation itself fails, the
run reaches the `CompensationFailed` terminal — fail loudly, persist the `Trace` journal, emit restart instructions (see
...
type RecoveryPayload struct {
    // Complement is the state captured by Do, passed to Undo on rollback.
...
StartTime, Duration, Error. It ignores `Recovery.Complement` (which may be
...
- [ ] Update `Action.Do` to return `(Result, Complement, ReconciliationState, error)`
