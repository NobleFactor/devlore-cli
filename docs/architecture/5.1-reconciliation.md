...
```go
Do(ctx *Context, slots map[string]any) (Result, Compensator, ReconciliationState, error)
```
...
| Compensator | Recovery Stack | Mechanical reversal if the graph fails |
...
#### Reconcile, `FailedCompensation`, and Restart

The reconcile gate is also what makes restart after a *failed* unwind safe. When a compensation itself fails, the
run reaches the `FailedCompensation` terminal — fail loudly, persist the `Trace` journal, emit restart instructions (see
[§5.2 Recovery Serialization](5.2-recovery-serialization.md) and
[compensation-failure-contract.md](../plans/extract-starlark-from-op/phase-8/compensation-failure-contract.md)).
On restart, the resumed `Unwind` runs the *same* reconcile→undo dance: it re-observes each resource's actual state
and undoes only what is still present. So an operator can fix the root cause and incidentally clear state by hand —
the resume observes reality rather than assuming the recorded plan, and converges to a clean baseline regardless.
...
type RecoveryPayload struct {
    // Compensator is the state captured by Do, passed to Undo on rollback.
...
StartTime, Duration, Error. It ignores `Recovery.Compensator` (which may be
...
- [ ] Update `Action.Do` to return `(Result, Compensator, ReconciliationState, error)`
