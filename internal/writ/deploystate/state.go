...
type State struct {
	...
	Signature *execution.Signature `json:"signature,omitempty" yaml:"signature,omitempty"`
}
...
// UpdateFromGraph merges nodes from an execution graph into the state.
func (s *State) UpdateFromGraph(g *execution.Graph, receiptFilename string) {
	for _, n := range g.Nodes {
		// Skip non-file nodes
		if n.Status == execution.StatusSkipped {
			continue
		}
		// Skip backup-only nodes
		if len(n.Operations) == 1 && n.Operations[0] == "backup" {
			continue
		}
