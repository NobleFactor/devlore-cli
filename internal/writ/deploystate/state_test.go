...
func TestStateUpdateFromGraph(t *testing.T) {
	s := New("/home/user/environment", "/home/user")
	...
	s.UpdateFromGraph(g, "2026-01-21T10-30-00.yaml")

	// Should have 2 files: .bashrc and .gitconfig
	// backup-only and skipped nodes should be excluded
	if len(s.Files) != 2 {
	...
	if len(gitconfig.Operations) != 2 || gitconfig.Operations[0] != "expand" {
		t.Errorf("expected operations ['expand', 'copy'], got %v", gitconfig.Operations)
	}

	// Backup-only and skipped should not be in state
	if s.GetEntry(".config/packages.manifest") != nil {
		t.Error("expected backup-only node to be excluded from state")
	}
