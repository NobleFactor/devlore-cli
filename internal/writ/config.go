...
func parseConflictResolution(flag string) (execution.ConflictResolution, error) {
	switch flag {
	case "stop", "":
		return execution.ResolutionStop, nil
	case "backup":
		return execution.ResolutionBackup, nil
	case "overwrite":
		return execution.ResolutionOverwrite, nil
	case "skip":
		return execution.ResolutionSkip, nil
	default:
		return execution.ResolutionStop, fmt.Errorf("invalid --conflict value %q: must be stop, backup, overwrite, or skip", flag)
	}
}
