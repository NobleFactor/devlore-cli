...
func linkToLayer(sourceRoot, layerDir string, verbose bool) error {
	...
	if _, err := fp.Mkdir(file.Resource{SourcePath: filepath.Dir(layerDir)}, 0o755); err != nil {
		return err
	}
	...
	_, _, err := fp.Link(file.Resource{SourcePath: sourceRoot}, file.Resource{SourcePath: layerDir})
	return err
}
...
func moveToLayer(sourceRoot, layerDir string, verbose bool) error {
	...
	_, _, err := fp.Move(file.Resource{SourcePath: sourceRoot}, file.Resource{SourcePath: layerDir})
	return err
}
