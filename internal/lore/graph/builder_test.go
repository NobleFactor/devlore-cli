...
func TestEngineRunsPackageInstallOperations(t *testing.T) {
	...
	eng := engine.New(reg, engine.Options{DryRun: true})
	...
	if results[0].Status != engine.StatusCompleted {
...
func TestDarwinPackageManagerPrefix(t *testing.T) {
	// Test that brew: and port: prefixes are correctly parsed
	// This tests the prefix parsing logic for macOS dual package managers
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			pkg, prefix := registry.ParsePackagePrefix(tt.input)
			if pkg != tt.wantPkg {
