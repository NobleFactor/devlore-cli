...
func TestResolveWithConfidence_LorePackage(t *testing.T) {
...
`
	...
	client := &Client{cacheDir: tmpDir}

	pkg, confidence, err := client.ResolveWithConfidence("testpkg", "Darwin")
	if err != nil {
...
func TestResolveWithConfidence_NativePackage(t *testing.T) {
	...
	pkg, confidence, err := client.ResolveWithConfidence("curl", "Darwin")
	if err != nil {
		t.Fatalf("ResolveWithConfidence() error = %v", err)
	}
...
`
	...
	// Search for "docker"
	results, err := client.searchLore("docker", 10)
	if err != nil {
	...
	results, err = client.searchLore("kube", 10)
	if err != nil {
	...
	results, err = client.searchLore("nonexistent", 10)
	if err != nil {
...
func TestListPackages(t *testing.T) {
	...
	results, err := client.ListPackages()
	if err != nil {
