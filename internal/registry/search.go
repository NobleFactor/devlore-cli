...
func (c *Client) Search(query string, opts SearchOptions) ([]SearchResultItem, error) {
	var results []SearchResultItem

	// Search lore registry first
	if opts.IncludeLore {
		loreResults, err := c.searchLore(query, opts.Limit)
		if err == nil {
	...
	if opts.IncludeNative {
		nativeResults := c.searchNative(query, opts.Limit)
...
func (c *Client) searchLore(query string, limit int) ([]SearchResultItem, error) {
...
func (c *Client) searchNative(query string, limit int) []SearchResultItem {
	h := host.NewHost()
	pm := h.PackageManager()
	if pm == nil {
		return nil
...
func (c *Client) ListPackages() ([]SearchResultItem, error) {
	packagesDir := filepath.Join(c.cacheDir, "packages")
	entries, err := os.ReadDir(packagesDir)
	if err != nil {
		return nil, err
...
func (c *Client) ResolveWithConfidence(name string, platform string) (*LorePackage, Confidence, error) {
	pkg, err := c.Resolve(name, platform)
	if err != nil {
