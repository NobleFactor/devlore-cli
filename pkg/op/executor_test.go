...
// testNode creates a node with the given receiver and source/target slots for testing.
func testNode(id string, receiver string, source, target string) *op.Node {
	node := &op.Node{ID: id, Receiver: receiver}
	if source != "" {
...
func TestAllProvidersCount(t *testing.T) {
	...
	for _, name := range expected {
		if _, ok := reg.ActionByName(name); !ok {
...
func TestLinkAction(t *testing.T) {
	...
	ctx := &op.ExecutionContext{contextBase: op.contextBase{Context: context.Background()}}
...
func TestRenderAction(t *testing.T) {
	...
	ctx := &op.ExecutionContext{contextBase: op.contextBase{Context: context.Background()}}
	node := &op.Node{ID: ".bashrc", Origin: "all"}
...
func TestWriteDryRun(t *testing.T) {
	...
	p := &file.Provider{}
	action := &filegen.WriteText{Impl: p}
	ctx := &op.ExecutionContext{contextBase: op.contextBase{Context: context.Background(), DryRun: true, Writer: io.Discard}}
	node := &op.Node{ID: "test"}
	node.SetSlotImmediate("destination", target)
...
func TestEngineRunLinkPipeline(t *testing.T) {
	...
	fp := &file.Provider{}
	engine := op.NewGraphExecutor()
	graph := &op.Graph{
		Children: []op.SubgraphChild{
			{Node: testNode(".bashrc", &filegen.Link{Impl: fp}, source, target)},
		},
	}
...
func TestEngineRunRenderCopyPipeline(t *testing.T) {
	...
	graph := &op.Graph{
		Children: []op.SubgraphChild{{Node: renderNode}, {Node: copyNode}},
	...
	for _, child := range graph.Children {
		if child.Node.Status != op.StatusCompleted {
			t.Fatalf("node %s: expected completed, got %s (error: %s)", child.Node.ID, child.Node.Status, child.Node.Error)
		}
	}
...
func TestEngineRunDecryptRenderCopyPipeline(t *testing.T) {
	...
	graph := &op.Graph{
		Children: []op.SubgraphChild{{Node: decryptNode}, {Node: renderNode}, {Node: copyNode}},
		Edges: []op.Edge{
...
func TestEngineRunMultipleNodes(t *testing.T) {
	...
	graph := &op.Graph{
		Children: []op.SubgraphChild{
			{Node: testNode("tgt1.txt", &filegen.Link{Impl: fp}, source1, target1)},
			{Node: testNode("sub/tgt2.txt", &filegen.Link{Impl: fp}, source2, target2)},
		},
	}
	...
	for _, child := range graph.Children {
		if child.Node.Status != op.StatusCompleted {
			t.Errorf("node %s: expected completed, got %s", child.Node.ID, child.Node.Status)
		}
	}
}
...
func TestEngineRunUnknownAction(t *testing.T) {
	...
	graph := &op.Graph{
		Children: []op.SubgraphChild{
			{Node: &op.Node{ID: "test", Receiver: "unknown_op"}},
		},
	}
...
func TestEngineTopologicalSort(t *testing.T) {
	...
	graph := &op.Graph{
		Children: []op.SubgraphChild{
			{Node: testNode("c", &filegen.Link{Impl: fp}, srcC, filepath.Join(tmpDir, "out_c"))},
			{Node: testNode("a", &filegen.Link{Impl: fp}, srcA, filepath.Join(tmpDir, "out_a"))},
			{Node: testNode("b", &filegen.Link{Impl: fp}, srcB, filepath.Join(tmpDir, "out_b"))},
		},
		Edges: []op.Edge{
			{From: "a", To: "b"},
			{From: "b", To: "c"},
	...
	// All nodes should complete (edges enforce a → b → c ordering).
	for _, child := range graph.Children {
		if child.Node.Status != op.StatusCompleted {
			t.Errorf("node %s: expected completed, got %s", child.Node.ID, child.Node.Status)
		}
...
func TestPreflightNoConflict(t *testing.T) {
	...
	graph := &op.Graph{
		Children: []op.SubgraphChild{
			{Node: testNode("test", "file.link", source, target)},
		},
	}

	result := op.Preflight(graph)
...
func TestPreflightConflictRegularFile(t *testing.T) {
	...
	if result.Conflicts[0].Type != op.ConflictRegularFile {
...
func TestResultStatusString(t *testing.T) {
	tests := []struct {
		status op.ResultStatus
		want   string
	}{
		{op.ResultPending, "pending"},
		{op.ResultRunning, "running"},
		{op.ResultCompleted, "completed"},
		{op.ResultFailed, "failed"},
		{op.ResultSkipped, "skipped"},
	}
