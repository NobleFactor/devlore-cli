...
// functionUnmarshaler passes through a *starlark.Function to a target of
// that exact pointer type.
//
// The canonical Go target for a starlark function is *mem.Function, but
// converting to that shape requires an ExecutionContext and a namespace
// (uri), neither of which an Unmarshaler has access to. The op-side
// Converter system (introduced in step 4.b) handles the ctx-aware
// conversion from *starlark.Function to *mem.Function after this
