...
func resolveInput(value starlark.Value) (*Promise, error) {
	if output, ok := value.(*Promise); ok {
		return output, nil
	}
	return nil, fmt.Errorf("expected Promise, got %s", value.Type())
}
...
func TestOutputString(t *testing.T) {
	tests := []struct {
		...
		{"simple id", "n1", "Promise(n1)"},
		{"dotted id", "file.copy.1", "Promise(file.copy.1)"},
		{"path id", "/tmp/foo", "Promise(/tmp/foo)"},
	}
...
func TestFillSlotMixedListFallsThrough(t *testing.T) {
	g := makeTestGraph()
	// A list containing non-Promise values should fall through to immediate handling.
	...
	if !sv.IsImmediate() {
		t.Error("expected immediate slot for non-Promise list")
	}
}
...
func TestResolveInputNonOutput(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "expected Promise") {
		t.Errorf("error = %q, want to contain %q", err.Error(), "expected Promise")
	}
}
...
func TestOutputRetryBuiltin(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if result != out {
				t.Error("retry() should return the Promise for chaining")
			}
