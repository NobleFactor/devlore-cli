...
func TestArchiveFile_CreatesRecoveryDir(t *testing.T) {
	...
	// Recovery ID is an opaque UUID v7; presence under .devlore/recovery/ is verified by RestoreFile and content-read
...
func TestArchiveStream_LargeReader(t *testing.T) {
	...
	// Exercise streaming by writing a payload larger than any reasonable in-memory default buffer. 1 MiB is enough to
	// verify io.Copy drains chunk-by-chunk.
...
// erroringReader yields `after` bytes of zeros and then returns a synthetic error.
//
// It exercises the mid-stream error path in ArchiveStream.
