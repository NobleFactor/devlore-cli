...
//
// Freshness guaranteed: the file is opened and read whenever the function is invoked. Errors with [op.ErrUnimplemented]
// for directories; the catch-all file.Resource pre-dates the taxonomic split into Regular / Directory / Link variants
// and directory hashing requires a Merkle-root scheme deferred until that split (step 22).
//
// Returns:
...
//
// Exact match: the other operand must represent a genuine file, not merely a general resource that shares the same
// address. Once the type check passes, URI comparison is delegated to [op.ResourceBase.Equal]. A cross-type URI
// collision (e.g., a file URI embedded in an appnet.Resource) fails at the type check rather than matching spuriously.
//
// Parameters:
...
// Etag provides an inexpensive change-detection token based on statistics
//
// The value returned is a SHA256 hash of (size, mtime_ns, ino) arranged in little-endian order. It is Always fresh
// because it `stats` the file at call time, ignoring any resolved-time fields cached on the Resource. The catalog
// uses Etag as an inexpensive signal that a full [Resource.Digest] comparison should be performed.
//
// Returns:
...
//
// The path is canonical from construction; rebinding updates Rel for confined I/O under the execution root. If the file
// does not exist, Resolve returns nil — existence is observation, not identity, and `not-exist` is a valid observation
// outcome. Other stat failures (permission denied, I/O error) surface as errors.
//
...
// UnmarshalJSON fills the receiver with data from a JSON-encoded string, which can be a file path or a file URI.
//
...
// UnmarshalText initializes the receiver using raw UTF-8 data representing a file path or URI.
//
// The caller pre-seeds the receiver's embedded [op.ResourceBase] with a valid [op.RuntimeEnvironment] before invoking
// this method; all domain-specific fields are then overwritten by the reconstructed resource.
//
...
// UnmarshalYAML fills the receiver with data from a YAML scalar, which can be a file path or a file URI.
//
// The caller pre-seeds the receiver's embedded [op.ResourceBase] with a valid [op.RuntimeEnvironment] before invoking
// this method; all domain-specific fields are then overwritten by the reconstructed resource.
//
...
//
// Returns:
//   - *Resource: the constructed candidate. Not interned in the catalog — callers ([NewResource] /
//     [DiscoverResource]) route it through [op.ResourceCatalog] themselves.
