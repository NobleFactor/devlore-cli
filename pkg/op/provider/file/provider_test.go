...
func TestCompensateLink_NewSymlink_RemovesOnCompensate(t *testing.T) {
	...
	p := testProvider(t, tmp)
	if err := p.CompensateFileMutation(testActivation(t, p.RuntimeEnvironment()), state); err != nil {
...
func TestBackup_MovesFileToTimestampedBackup(t *testing.T) {
	...
	resourcePath := state.Resource().(Resource).Path().Abs()
...
func TestMove_MovesFileToDestination(t *testing.T) {
	...
	p := testProvider(t, tmp)
	result, state, err := p.Move(testActivation(t, p.RuntimeEnvironment()), mustDiscoverRegular(t, p, src), dst)
	if err != nil {
...
func TestCompensateMove_RoundTrip(t *testing.T) {
	...
	p := testProvider(t, tmp)
	_, state, err := p.Move(testActivation(t, p.RuntimeEnvironment()), mustDiscoverRegular(t, p, src), dst)
	if err != nil {
		t.Fatalf("Move() error = %v", err)
	}
...
func TestCompensateRemove_RoundTrip(t *testing.T) {
	...
	if state.Resource().(Resource).Path().Abs() != path {
		t.Errorf("receipt resource path = %q, want %q (true identity)", state.Resource().(Resource).Path().Abs(), path)
	}
...
func TestCompensateRemoveAll_RoundTrip(t *testing.T) {
	...
	if state.Resource().(Resource).Path().Abs() != dir {
		t.Errorf("receipt resource path = %q, want %q (true identity)", state.Resource().(Resource).Path().Abs(), dir)
	}
...
func TestCompensateMove_RoundTrip_RemovesCreatedParents(t *testing.T) {
	...
	p := testProvider(t, tmp)
	_, receipt, err := p.Move(testActivation(t, p.RuntimeEnvironment()), mustDiscoverRegular(t, p, source), dest)
	if err != nil {
		t.Fatalf("Move() error = %v", err)
	}
