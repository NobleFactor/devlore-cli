...
func newWindows() *op.Platform {
	...
	return &op.Platform{
		...
		PackageManagers: map[string]op.PackageManager{"winget": winget},
		ServiceManager:  &windowsServiceManager{},
	}
}

// runWindowsCommand executes a command via PowerShell or cmd.
func runWindowsCommand(command string, elevated bool) op.PlatformResult {
	var cmd *exec.Cmd
...
func (m *wingetManager) Search(query string, limit int) []op.SearchResult {
...
func (m *wingetManager) Install(packages ...string) op.PlatformResult {
...
func (m *wingetManager) Remove(name string) op.PlatformResult {
	return runWindowsCommand("winget uninstall --id "+name, false)
}

func (m *wingetManager) Update() op.PlatformResult {
	return runWindowsCommand("winget upgrade", false)
}

func (m *wingetManager) AddRepo(url, _, name string) op.PlatformResult {
...
func (m *windowsServiceManager) Start(name string) op.PlatformResult {
	return runWindowsCommand("sc start "+name, true)
}

func (m *windowsServiceManager) Stop(name string) op.PlatformResult {
	return runWindowsCommand("sc stop "+name, true)
}

func (m *windowsServiceManager) Enable(name string) op.PlatformResult {
	return runWindowsCommand("sc config "+name+" start= auto", true)
}

func (m *windowsServiceManager) Disable(name string) op.PlatformResult {
