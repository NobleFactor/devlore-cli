...
func newDarwin() *op.Platform {
	...
	var preferred op.PackageManager
...
func (m *brewManager) Search(query string, limit int) []op.SearchResult {
	...
	for _, line := range lines {
		...
		for _, pkg := range strings.Fields(line) {
			...
			results = append(results, op.SearchResult{Name: pkg})
...
func (m *brewManager) Install(packages ...string) op.PlatformResult {
	return runShellCommand("brew install "+strings.Join(packages, " "), false)
}

func (m *brewManager) Remove(name string) op.PlatformResult {
	return runShellCommand("brew uninstall "+name, false)
}

func (m *brewManager) Update() op.PlatformResult {
	return runShellCommand("brew update", false)
}

func (m *brewManager) AddRepo(url, keyURL, name string) op.PlatformResult {
...
func (m *portManager) Search(query string, limit int) []op.SearchResult {
	...
	for _, line := range lines {
		...
		if len(fields) >= 1 {
			sr := op.SearchResult{Name: fields[0]}
			if len(fields) >= 2 {
...
func (m *portManager) Install(packages ...string) op.PlatformResult {
	return runShellCommand("port install -N "+strings.Join(packages, " "), true)
}

func (m *portManager) Remove(name string) op.PlatformResult {
	return runShellCommand("port uninstall "+name, true)
}

func (m *portManager) Update() op.PlatformResult {
	return runShellCommand("port selfupdate", true)
}

func (m *portManager) AddRepo(_, _, _ string) op.PlatformResult {
	return op.PlatformResult{OK: false, Stderr: "MacPorts doesn't support dynamic repository addition"}
}
...
func (m *launchdManager) Start(name string) op.PlatformResult {
	return runShellCommand("launchctl start "+name, false)
}

func (m *launchdManager) Stop(name string) op.PlatformResult {
	return runShellCommand("launchctl stop "+name, false)
}

func (m *launchdManager) Enable(name string) op.PlatformResult {
...
func (m *launchdManager) Disable(name string) op.PlatformResult {
	userPlist := "~/Library/LaunchAgents/" + name + ".plist"
	systemPlist := "/Library/LaunchDaemons/" + name + ".plist"
