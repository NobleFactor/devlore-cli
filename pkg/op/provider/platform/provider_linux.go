...
func newLinux() *op.Platform {
	...
	packageManagers := make(map[string]op.PackageManager)
...
func detectLinuxPackageManager(distro string) op.PackageManager { //nolint:ireturn // returns concrete behind interface
	switch distro {
...
func (m *aptManager) Search(query string, limit int) []op.SearchResult {
	result := runShellCommand("apt-cache search "+query, false)
	if !result.OK {
		return nil
	}

	var results []op.SearchResult
	lines := strings.Split(result.Stdout, "\n")
	for _, line := range lines {
		parts := strings.SplitN(line, " - ", 2)
		if len(parts) >= 1 && parts[0] != "" {
			sr := op.SearchResult{Name: strings.TrimSpace(parts[0])}
			if len(parts) >= 2 {
...
func (m *aptManager) Install(packages ...string) op.PlatformResult {
	return runShellCommand("apt-get install -y "+strings.Join(packages, " "), true)
}

func (m *aptManager) Remove(name string) op.PlatformResult {
	return runShellCommand("apt-get remove -y "+name, true)
}

func (m *aptManager) Update() op.PlatformResult {
	return runShellCommand("apt-get update", true)
}

func (m *aptManager) AddRepo(url, keyURL, name string) op.PlatformResult {
	if keyURL != "" {
...
func (m *dnfManager) Search(query string, limit int) []op.SearchResult {
	...
	for _, line := range lines {
		...
		if len(parts) >= 1 {
			...
			sr := op.SearchResult{Name: namePart}
			if len(parts) >= 2 {
				sr.Description = strings.TrimSpace(parts[1])
			}
			results = append(results, sr)
...
func (m *dnfManager) Install(packages ...string) op.PlatformResult {
	return runShellCommand("dnf install -y "+strings.Join(packages, " "), true)
}

func (m *dnfManager) Remove(name string) op.PlatformResult {
	return runShellCommand("dnf remove -y "+name, true)
}

func (m *dnfManager) Update() op.PlatformResult {
	return runShellCommand("dnf check-update || true", true)
}

func (m *dnfManager) AddRepo(url, keyURL, _ string) op.PlatformResult {
	if keyURL != "" {
...
func (m *pacmanManager) Search(query string, limit int) []op.SearchResult { //nolint:gocognit // parsing format requires nesting
	...
	for i := 0; i < len(lines); i++ {
		...
		if len(parts) >= 1 {
			...
			if idx := strings.Index(repoAndName, "/"); idx >= 0 {
				...
				sr := op.SearchResult{Name: pkgName}
...
func (m *pacmanManager) Install(packages ...string) op.PlatformResult {
	return runShellCommand("pacman -S --noconfirm --needed "+strings.Join(packages, " "), true)
}

func (m *pacmanManager) Remove(name string) op.PlatformResult {
	return runShellCommand("pacman -R --noconfirm "+name, true)
}

func (m *pacmanManager) Update() op.PlatformResult {
	return runShellCommand("pacman -Sy", true)
}

func (m *pacmanManager) AddRepo(url, keyURL, name string) op.PlatformResult {
	if keyURL != "" {
...
func (m *systemdManager) Start(name string) op.PlatformResult {
	return runShellCommand("systemctl start "+name, true)
}

func (m *systemdManager) Stop(name string) op.PlatformResult {
	return runShellCommand("systemctl stop "+name, true)
}

func (m *systemdManager) Enable(name string) op.PlatformResult {
	return runShellCommand("systemctl enable "+name, true)
}

func (m *systemdManager) Disable(name string) op.PlatformResult {
