...
// Nodes and subgraphs share one ID space and are interchangeable wherever a
// reference is valid (Phase 7 invariant).
//
// The parameter surface is precomputed at plan time and immutable afterward;
// Execute trusts the surface and does not revalidate.
...
// executableUnit is the shared state embedded by Node and Subgraph. Both
// fields are unexported; callers read through the ID() and Parameters()
// methods and write through the constructors (NewNode, NewSubgraph) and
// plan-time hooks (Node.Bind, Subgraph.FinalizeParameters).
...
//
// The surface is the union by name of every topological root's parameters,
// where a root is a child with no incoming edges from within this subgraph.
// Name collisions across roots are a plan-time error: if two roots declare
// the same parameter name with different types, FinalizeParameters returns
// an error and the plan is rejected.
//
// Recursive: if a root is itself a subgraph, its FinalizeParameters must
// have been called first (or be safe to call now; this method invokes it
// via Parameters()).
...
// childParameters returns the parameter surface of a SubgraphChild by
// delegating to whichever field is populated. A child subgraph must have
// been finalized (Parameters() returns non-nil if it has roots, nil
// otherwise — an empty subgraph legitimately exposes no parameters).
