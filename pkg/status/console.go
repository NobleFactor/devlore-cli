...
//
// Parameters:
//   - writer: output destination, typically [os.Stderr]. Must not be nil.
//   - programName: name shown in the "[program]" prefix on categorized messages (e.g., "lore").
//   - color: when true, ANSI color codes wrap the status symbol; when false, symbols emit as plain bytes. Callers
//     typically derive this from a TTY check on w.
//   - silent: when true, every method (including [Console.Print] and [Console.Fail]) emits nothing; [Console.Fail]
//     still returns a non-nil error so callers can propagate the failure.
//
// Returns:
...
//
// Silent mode suppresses the emission but still returns the error.
...
func (c *Console) Succeed(msg string) {
...
// emit writes one decorated status line to the writer.
//
