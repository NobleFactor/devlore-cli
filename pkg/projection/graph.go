...
// Package projection owns the concrete graph data model types shared by
// the execution engine, Starlark projection layer, and CLI tools.
//
// # Core Types
//
...
func StubAction(name string) any { return &stubAction{name: name} }
...
func (n *Node) ActionName() string {
	if n.Action != nil {
		if named, ok := n.Action.(interface{ Name() string }); ok {
			return named.Name()
		}
	}
	return ""
